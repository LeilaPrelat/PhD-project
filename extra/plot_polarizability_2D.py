...
err = 'polarizability.py no se encuentra en ' + path_basic
try:
...
labelx,labely1,labely2 = '$\omega/c$ [1/$\mu$m]', 'Re $\alpha$', 'Im $\alpha$'
...
for omegac in list_omegac: 
    valuey = alpha_function(omegac,hbarmu,z_plane,epsi1,epsi2,hbargama)
...
if save_graphs==1:
    plt.tight_layout(1)
    os.chdir(path_save)
    plt.savefig( 'polarizability_Re' +  labelpng + '.png', format='png')
...
if save_graphs==1:
    plt.tight_layout(1)
    os.chdir(path_save)
    plt.savefig( 'polarizability_Im' +labelpng + '.png', format='png')
