...
############ I0 5 ##################################
##################################################################
    Int06_B_function_re = lambda u: np.real(rp(u)*(J0(u) + np.cos(2*phi)*J2(u))*expB(u)*u**2 + rs(u)*(J0(u) - np.cos(2*phi)*J2(u)))
    Int06_B_function_im = lambda u: np.imag(rp(u)*(J0(u) + np.cos(2*phi)*J2(u))*expB(u)*u**2 + rs(u)*(J0(u) - np.cos(2*phi)*J2(u)))
...
#    cte1 = np.sqrt(n1)
    ...
    H2 = special.hankel1(2,arg)


    final = np.pi*1j*0.5*Rp*alphap_3*k1_3*(H0 + np.cos(2*phi)*H2)*expo
