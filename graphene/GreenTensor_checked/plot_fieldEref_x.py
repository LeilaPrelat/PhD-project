...
err = 'fieldE_ref_x.py no se encuentra en ' + path_basic

try:
    sys.path.insert(1, path_basic)
    from fieldE_ref_x import Efield_ref_num,Efield_ref_ana,Efield_ref_fresnel
except ModuleNotFoundError:
    print(err)


path_save = path_basic + '/' + 'fieldE_ref_x'
...
title = r'z = %i nm, $z_e$ = %i nm, $z_p$ = %i nm, v = c/%i' %(z*1e3,ze*1e3,zp*1e3,int_v)
########################
if plot_vs_E == 1:
#    omegaTHz0 = 0.7
#    omegaTHz0 = 1.511
    ...
    R0 = 10
    listx = np.linspace(37,50,N)

    labelx = '$\hbar\omega$ [meV]'
    label1 = '_vs_E'
    title = title +  r', x = %i nm' %(R0)
...
def Efield_ANA_QE_function(energy0,x_nano):
    ...
    return Efield_ref_ana(omegac0,epsi1,epsi2,hbmu,hbgama,x,z,ze,zp,int_v)
...
def Efield_NUM_QE_function(energy0,x_nano):
    ...
    return Efield_ref_num(omegac0,epsi1,epsi2,hbmu,hbgama,x,z,ze,zp,int_v)
...
def Efield_NUM_fresnel_function(energy0,x_nano):
    ...
    return Efield_ref_fresnel(omegac0,epsi1,epsi2,hbmu,hbgama,x,z,ze,zp,int_v)
...
plt.plot(listx,total2_re,'.-',ms = ms,color = 'darkred',label = 'analytical')
# plt.yscale('log')
#plt.plot(listx,total1_re,'.',ms = ms,color = 'green',label = 'numerical PP')
...
plt.plot(listx,total2_im,'.-',ms = ms,color = 'darkred',label = 'analytical')
# plt.yscale('log')
#plt.plot(listx,total1_im,'.',ms = ms,color = 'green',label = 'numerical PP')
