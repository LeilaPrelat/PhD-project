...
#def green_tensor_ref_fresnel_rp(omegac,epsi1,epsi2,hbmu,hbgama,R,phi,zp):
#    """
#    Parameters
#    ----------
...
############# I0 5 ##################################
###################################################################
#    Int06_B_function_re = lambda u: np.real(rp(u)*(J0(u) - np.cos(2*phi)*J2(u))*expB(u)*u**2)
#    Int06_B_function_im = lambda u: np.imag(rp(u)*(J0(u) - np.cos(2*phi)*J2(u))*expB(u)*u**2)
#
...
def green_tensor_ref_fresnel(omegac,epsi1,epsi2,hbmu,hbgama,zp):
    """
    Parameters
    ----------
    omegac : omega/c = k0 en 1/micrometros    
...
############ I0 5 ##################################
##################################################################
    Int06_B_function_re = lambda u: np.real(rp(u)*expB(u)*u**2)
    Int06_B_function_im = lambda u: np.imag(rp(u)*expB(u)*u**2)
...
def green_tensor_ref_pole_aprox(omegac,epsi1,epsi2,hbmu,hbgama,zp):
    """
    Parameters
    ----------
    omegac : omega/c = k0 en 1/micrometros    
...
def green_tensor_PP1(omegac,epsi1,epsi2,hbmu,hbgama,zp):
    """
    Parameters
    ----------
    omegac : omega/c = k0 en 1/micrometros    
...
#    cte1 = np.sqrt(n1)
    ...
    final = 2*1j*np.pi*Rp*np.exp(-2*kp*zp)*kp_3
    
    return final



def green_tensor_PP2(omegac,epsi1,epsi2,hbmu,hbgama,zp):
    """
    Parameters
    ----------
    omegac : omega/c = k0 en 1/micrometros    
...
#    cte1 = np.sqrt(n1)
    ...
    Exp = special.expi(-2*kp*zp)
    
    

    final = Rp*kp*0.5*( (1 + 2*kp*zp)/(4*(zp**2)) + (kp**2)*Exp*np.exp(-2*kp*zp) )
