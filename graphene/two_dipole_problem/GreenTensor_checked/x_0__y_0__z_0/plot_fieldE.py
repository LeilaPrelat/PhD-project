...
plot_at_position_0 = 1  #no esta funcionando bien. va mas rapido pero la diferencia es mayor
...
path_constants =  path_basic.replace('/GreenTensor_checked/x_0__y_0__z_0','')
err = 'fieldE_direct_numerical.py no se encuentra en ' + path_basic
if plot_at_position_0 == 0:
    path_2 = path_basic.replace('/x_0__y_0__z_0','')
    try:
        sys.path.insert(1, path_2)
        from fieldE_direct_numerical import Efield_NUM_QE
...
try:
    sys.path.insert(1, path_basic)
    from fieldE_direct_analyticalQE import Efield_checked
except ModuleNotFoundError:
    print(err2)
...
#v = c/den
epsi1 = 1
x,y,z = 0,0,0 ## NO PUEDE CAMBIAR. SOLUCION ANALITICA SOLO PARA POSICION NULA. 
#omega = 0.7*1e12

if plot_vs_E == 1:
    list_x = np.linspace(0.01,2.01,251)
    list_x = np.linspace(0.01,50,200)
#    list_x = np.linspace(1,1.08,17)
    b = -10*1e-3 #electron position in z < 0 (arriba del plano)
    label1 = 'vs_E_v%i' %(int_v)
    labelx = 'E [meV]'
#    header0 = 'E [meV]'
    title = r'$\epsilon_1$ = %i, v = c/%i, b = %i nm' %(epsi1,int_v,b*1e3) 
...
if plot_vs_b == 1:
    ...
    labelx = 'b [$\mu$m]'
#    header0 = labelx
    title = r'$\epsilon_1$ = %i, v = c/%i, $\omega$ = %.2fTHz' %(epsi1,int_v,omegaTHz) 
...
if plot_vs_E == 1:
    for E in list_x: 
        ...
        omegac =  E*1e-3/aux  
        v1 = Efield_checked(omegac,epsi1,int_v,b)
        
        list_v1_re.append(v1.real)
        list_v1_im.append(v1.imag)
    
        if plot_at_position_0 == 0:
            v2 = Efield_NUM_QE(omegac,epsi1,int_v,b,x,y,z)
        else:
        ...
        print(j,'imag:', v1.imag,v2.imag)
        print(j,'real:', v1.real,v2.real)
