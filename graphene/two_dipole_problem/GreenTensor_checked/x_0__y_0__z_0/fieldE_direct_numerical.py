...
def Efield_NUM_QE(omegac,epsi1,int_v,ze):
    """
    Parameters
    ----------
...
#    yD,zD = 0, b
    ...
    z_dip_barra = k1*np.abs(ze)               # green tensor evaluated in x,y,z = 0, 0, 0 and yD,zD = 0, b
    
    expB = lambda u: np.exp(-u*z_dip_barra) 
    
#    sin_electron = lambda w: np.sin(w*int_v/n1)
    cos_electron = lambda w: np.cos(w*int_v)
    ...
    cota_inf = 1*k1
############ I0 5 ########################################################
    Int05_B_function_re = lambda w,u: np.real(expB(u)*cos_electron(w)*(J0(w,u) + J2(w,u))*(u**2 - 1))
    Int05_B_function_im = lambda w,u: np.imag(expB(u)*cos_electron(w)*(J0(w,u) + J2(w,u))*(u**2 - 1))

    int05B_re,err = integrate.dblquad(Int05_B_function_re, cota_inf, cota_sup1, cota_inf, cota_sup2)
    int05B_im,err = integrate.dblquad(Int05_B_function_im, cota_inf, cota_sup1, cota_inf, cota_sup2)
#########################################################################

    charge_e = 1.602*1e-19
    charge_e = 1
    v = c/int_v
    cte_aux = 1j*charge_e*k1_2*np.sign(v)/(omega)

#    cte = k1_2*cte_aux #signo menos
    rta05 = (int05B_re + 1j*int05B_im)*cte_aux 
