...
############ I0 5 ###################################
    Int05_B_function_re = lambda u: np.real((J0(u)*(1-u**2) + np.cos(2*phi)*J2(u)*(1 + u**2))*expB(u))
    Int05_B_function_im = lambda u: np.imag((J0(u)*(1-u**2) + np.cos(2*phi)*J2(u)*(1 + u**2))*expB(u))
...
#    phi = np.arctan2(np.abs(y-ye),np.abs(x-xe))
    ...
    term2 = 1 + np.cos(2*phi)*(np.abs(z-ze)/R)**2
    
    term3 = den**(1/2) - 2*np.abs(z-ze)
    
    return term1/(den**(5/2)) + term2*k1_2*0.5/(den**(1/2)) + term3*0.5*k1_2*np.cos(2*phi)/R_2
