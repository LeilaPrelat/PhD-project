...
def Efield_NUM_QE(omegac,epsi1,int_v,b):
    """
    Parameters
    ----------
...
#    phi = 0  # green tensor evaluated in 
    atan = lambda w : np.cos(np.pi - 2*np.arctan(np.abs(w))) # me habia olvidado de este factor 
...
def Efield_NUM(omegac,epsi1,int_v,b):
    """
    Parameters
    ----------
    omegac : omega/c = k0 en 1/micrometros    
...
############ I0 5 ########################################################
    ...
    Int05_B_function_re = lambda w,u: np.real(1j*(J0(w,u) + atan(w)*J2(w,u))*u*expB(u)*exp_electron(w)*(alpha_z2(u) - 1/alpha_z2(u)))
    Int05_B_function_im = lambda w,u: np.imag(1j*(J0(w,u) + atan(w)*J2(w,u))*u*expB(u)*exp_electron(w)*(alpha_z2(u) - 1/alpha_z2(u)))
