...
path_constants =  path_basic.replace('/comparar_GreenTensor_electrostatic','')
...
try:
    ...
    from fieldEtot_GreenTensor import Efield_ana1b
except ModuleNotFoundError:
    print(err)
err2 = 'fieldE_direct_analyticalQE.py no se encuentra en ' + path_basic
try:
    sys.path.insert(1, path_basic)
    from fieldEtot_GreenTensor import Efield_ANA_QE_2terms
except ModuleNotFoundError:
    print(err2)
...
for omegaTHz in list_OmegaTHz: 
    ...
    v = v1 + v2
    list_v_re_GT.append(v.real)
    list_v_im_GT.append(v.imag)
...
plt.plot(list_E,list_v_re_GT,'.',ms = ms,color = 'lightseagreen',label = 'Green Tensor')  # plt.yscale('log')
...
plt.plot(list_E,list_v_im_GT,'.',ms = ms,color = 'lightseagreen',label = 'Green Tensor')
# plt.yscale('log')
plt.legend(loc = 'best',markerscale=mk,fontsize=tamlegend,frameon=True,handletextpad=hp, handlelength=1)
if save_graphs==1:
    plt.tight_layout()
