...
diferencia con la version1 : devuelve los 2 terminos juntos
"""
...
#    phi = 0  # green tensor evaluated in 
    ...
    cota_sup2 = 60
############ I0 5 ########################################################
    Int05_B_function_re = lambda w,u: np.real(J0(w,u)*expB(u)*exp_electron(w)*(u**2 - 1))
    Int05_B_function_im = lambda w,u: np.imag(J0(w,u)*expB(u)*exp_electron(w)*(u**2 - 1))

    int05B_re,err = integrate.dblquad(Int05_B_function_re, 0, cota_sup1, lambda u: -cota_sup2, lambda u: cota_sup2)
    int05B_im,err = integrate.dblquad(Int05_B_function_im, 0, cota_sup1, lambda u: -cota_sup2, lambda u: cota_sup2)
#########################################################################
...
############ I0 5 ########################################################
    Int05_A_function_re = lambda w,u: np.real(J0(w,u)*u*expA(u)*exp_electron(w)*(alpha_z1(u) + 1/alpha_z1(u)))
    Int05_A_function_im = lambda w,u: np.imag(J0(w,u)*u*expA(u)*exp_electron(w)*(alpha_z1(u) + 1/alpha_z1(u)))

    int05A_re,err = integrate.dblquad(Int05_A_function_re, 0, cota_sup1, lambda u: -cota_sup2A, lambda u:cota_sup2A)
    int05A_im,err = integrate.dblquad(Int05_A_function_im, 0, cota_sup1, lambda u: -cota_sup2A, lambda u:cota_sup2A)

    Int05_B_function_re = lambda w,u: np.real(1j*J0(w,u)*u*expB(u)*exp_electron(w)*(alpha_z2(u) - 1/alpha_z2(u)))
    Int05_B_function_im = lambda w,u: np.imag(1j*J0(w,u)*u*expB(u)*exp_electron(w)*(alpha_z2(u) - 1/alpha_z2(u)))

    int05B_re,err = integrate.dblquad(Int05_B_function_re, cota_inf1, cota_sup1A, lambda u:-cota_sup2A, lambda u:cota_sup2A)
    int05B_im,err = integrate.dblquad(Int05_B_function_im, cota_inf1, cota_sup1A, lambda u: -cota_sup2A, lambda u:cota_sup2A)
