...
path_save = path_basic + '/' + 'green_tensor_direct_functions'
...
try:
    ...
    from green_tensor_direct_functions import functions_GreenTensor
except ModuleNotFoundError:
    print(err)

try:
    sys.path.insert(1, path_basic)
    from green_tensor_direct_QE_functions import functions_QE_GreenTensor
except ModuleNotFoundError:
    print(err2)
...
list_alpha_parallel2 = np.linspace(1.5,3001.5,601)

list_alpha_parallel3 = np.linspace(1,3001,601) #for QE

def function(alpha_parallel):
    return functions_GreenTensor(omegac,epsi1,alpha_parallel,x,y,z,xD,yD,zD)

def functionQE(alpha_parallel):
    return functions_QE_GreenTensor(omegac,epsi1,alpha_parallel,x,y,z,xD,yD,zD)
...
if save_graphs==1:
    plt.tight_layout()
    os.chdir(path_save)
    plt.savefig( 'Re_green_tensor_dir' + '.png', format='png')
...
if save_graphs==1:
    plt.tight_layout()
    plt.savefig( 'Im_green_tensor_dir' + '.png', format='png')
...
if save_graphs==1:
    plt.tight_layout()
    os.chdir(path_save)
    plt.savefig( 'Re_green_tensorQE_dir' + '.png', format='png')
...
if save_graphs==1:
    plt.tight_layout()
    plt.savefig( 'Im_green_tensorQE_dir' + '.png', format='png')
