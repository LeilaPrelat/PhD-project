...
if type_plane == 'graphene':
    ...
    try:
        ...
        from green_tensor_ref_xx_graphene import green_tensor_NUM_QE, green_tensor_NUM, green_tensor_analitycal
    ...
    try:
        sys.path.insert(1, path_basic)
        from green_tensor_ref_xx_Ag import green_tensor_NUM_QE, green_tensor_NUM
    except ModuleNotFoundError:
        print(err)
...
if type_plane == 'graphene':
    ...
    def function_ANA(omegac,z,xD):
        return green_tensor_analitycal(omegac,epsi1,epsi2,hbmu,hbgama,x,y,z,xD,yD,zp)
...
title3 = r'$z_D$ = %i$\mu$m, $\hbar \mu$ = %.2feV, $\hbar \gamma$ = %.4feV, zp = %i$\mu$m' %(zD,hbmu,hbgama,zp) 
title = title1 + '\n' + title2 + '\n ' + title3
...
if plot_vs_z == 1:
    ...
    for z in list_z: 
        ...
        tot_ana = function_ANA(omegac0,z,xD0)
        total_ana_re.append(tot_ana.real)
        total_ana_im.append(tot_ana.imag)
    ...
    for OmegaTHz in list_OmegaTHz: 
        ...
        tot_ana = function_ANA(Omegac,z0,xD0)
        total_ana_re.append(tot_ana.real)
        total_ana_im.append(tot_ana.imag)        
    ...
    for xD in list_xD: 
        ...
        tot_ana = function_ANA(omegac0,z0,xD)
        total_ana_re.append(tot_ana.real)
        total_ana_im.append(tot_ana.imag)   
...
plt.plot(list_x,total_ana_re,'-',color = 'blue',label =  'analytical')
# plt.yscale('log')
plt.legend(loc = 'best',markerscale=mk,fontsize=tamlegend,frameon=True,handletextpad=hp, handlelength=1)
if save_graphs==1:
    plt.tight_layout()
...
plt.plot(list_x,total_ana_im,'-',color = 'blue',label =  'analytical')
# plt.yscale('log')
plt.legend(loc = 'best',markerscale=mk,fontsize=tamlegend,frameon=True,handletextpad=hp, handlelength=1)
if save_graphs==1:
    plt.tight_layout()
