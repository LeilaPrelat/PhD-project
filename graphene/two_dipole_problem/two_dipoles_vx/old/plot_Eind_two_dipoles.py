...
path_save = path_basic + '/' + 'Eind_dipoles'
...
try:
    ...
    from electric_ind_field import Efield1_ind_2dip
...
def f_alpha_function(omegac):
    return Efield1_ind_2dip(omegac,epsi1,epsi2,hbmu,hbgama,int_v,zp,b,xD1,yD1,zD1,xD2,yD2,zD2,omega0,kappa_factor_omega0,kappa_r_factor)
...
graph(title,labelx,r'Re($E_{ind,x}$)',tamfig,tamtitle,tamletra,tamnum,labelpadx,labelpady,pad)
...
if save_graphs==1:
    plt.tight_layout()
    os.chdir(path_save)
    plt.savefig( 'Re_Eind' + '.png', format='png')

graph(title,labelx,r'Im($E_{ind,x}$)',tamfig,tamtitle,tamletra,tamnum,labelpadx,labelpady,pad)
...
if save_graphs==1:
    plt.tight_layout()
    plt.savefig( 'Im_Eind' + '.png', format='png')


graph(title,labelx,r'|$E_{ind,x}$|',tamfig,tamtitle,tamletra,tamnum,labelpadx,labelpady,pad)
...
if save_graphs==1:
    plt.tight_layout()
    plt.savefig( '|Eind|' + '.png', format='png')
