...
try:
    ...
    from function_polarizability import alpha_function_eff_ana_2dipo
...
def f_alpha_function(omegac):
    return alpha_function_eff_ana_2dipo(omegac,epsi1,epsi2,hbmu,hbgama,int_v,zp,xD1,yD1,zD1,xD2,yD2,zD2,omega0,kappa_factor_omega0,kappa_r_factor)
...
title3 = r'xD1 = %inm, xD2 = %inm' %(xD1*1e3,xD2*1e3)
title = title1 + '\n' + title2  + '\n' + title3
...
if finding_peaks == 1:
    ...
    maxis_tot_x_re = list_meV[int(peaks_re)]
...
graph(title,labelx,r'Re($\alpha_{eff,eff}$)',tamfig,tamtitle,tamletra,tamnum,labelpadx,labelpady,pad)
...
if save_graphs==1:
    plt.tight_layout()
    os.chdir(path_save)
    plt.savefig( 'Re_polarizability_eff_eff' + '.png', format='png')

graph(title,labelx,r'Im($\alpha_{eff,eff}$)',tamfig,tamtitle,tamletra,tamnum,labelpadx,labelpady,pad)
...
if save_graphs==1:
    plt.tight_layout()
    plt.savefig( 'Im_polarizability_eff_eff' + '.png', format='png')
