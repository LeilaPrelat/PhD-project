...
title1 = r'$\kappa_1$ = %.2f$\omega_{01}$, $\kappa_{r1}$ = %.2f$\kappa$, $\hbar\omega_{01}$=%i meV, L = %i nm' %(kappa_factor_omega01, kappa_r_factor1, energy0_pol1,L*1e3)     
...
title4 = r'$x_1$ = %i nm, b = %i nm, zp = %i nm' %(x1*1e3,b*1e3,zp*1e3)
...
def function_num3D(energy0,x2_nano):
    ...
    px_f  = EELS_1_num(omegac0,epsi1,epsi2,hbmu,hbgama,int_v,b,zp,x1,z1,x2,z2,L,omega01,kappa_factor_omega01,kappa_r_factor1,omega02,kappa_factor_omega02,kappa_r_factor2)
    # print(rta)
    return np.real(px_f)
...
def function_ana3D(energy0,x2_nano):
    ...
    px_f  = EELS_1_ana(omegac0,epsi1,epsi2,hbmu,hbgama,int_v,b,zp,x1,z1,x2,z2,L,omega01,kappa_factor_omega01,kappa_r_factor1,omega02,kappa_factor_omega02,kappa_r_factor2)
...
im = plt.imshow(Z_ana, extent = limits, cmap=plt.cm.hot, aspect=1/100)

#divider = make_axes_locatable(plt)
#cax = divider.append_axes("right", size="5%", pad=0.05)
 
cbar = plt.colorbar(im, extend='both', fraction=1/25, pad=0.06)
cbar.ax.tick_params(labelsize = tamnum, pad = pad)
if paper == 0:
    cbar.set_label(labelz,fontsize=tamlegend,labelpad = 1)
    cbar.set_title(title,fontsize=int(tamtitle*0.9))
plt.tight_layout()
os.chdir(path_save)
