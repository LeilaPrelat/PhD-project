...
def green_tensor_NUM_QE(omegac,epsi1,epsi2,hbmu,hbgama,x,y,z,xD,yD,zD,zp):
    ...
    z_dip_barra = k1*np.abs(z + 2*zp + zD)
...
def green_tensor_analitycal(omegac,epsi1,epsi2,hbmu,hbgama,x,y,z,xD,yD,zp):
    """
    Parameters
    ----------
    omegac : omega/c = k0 en 1/micrometros    
    ...
    exp_gamma1 = np.exp(1j*(alpha_p*Rbarra-np.pi/4))
    exp_gamma2 = np.exp(1j*(np.pi/4 - alpha_p*Rbarra))
    exp_gamma0 = np.exp(alpha_p*np.abs(z+2*zp)*k1)
    
    gamma1 = special.gammaincc(-1/2,-alpha_p*np.abs(z + 2*np.abs(zp)-1j*Rbarra))
    gamma2 = special.gammaincc(-1/2,-alpha_p*np.abs(z + 2*np.abs(zp)+1j*Rbarra))
    
    
    cte = -1j*k1_3*Rp*(alpha_p**(3/2))/(16*np.sqrt(Rbarra))
    
    final = cte*exp_gamma0*(gamma1*exp_gamma1 + gamma2*exp_gamma2)
