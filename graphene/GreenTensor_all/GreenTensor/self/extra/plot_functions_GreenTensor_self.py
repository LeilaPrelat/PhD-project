...
path_constants =  path_basic.replace('GreenTensor/' + 'self/extra','')
path_save = path_basic + '/' + 'green_tensor_self_functions'
...
err = 'green_tensor_self_numerical.py no se encuentra en ' + path_basic
err2 =  'green_tensor_self_QE_numerical.py no se encuentra en ' + path_basic
try:
    sys.path.insert(1, path_basic)
    from green_tensor_self_functions import functions_GreenTensor
except ModuleNotFoundError:
    print(err)

try:
    sys.path.insert(1, path_basic)
    from green_tensor_self_QE_functions import functions_QE_GreenTensor
except ModuleNotFoundError:
    print(err2)
...
if save_graphs==1:
    plt.tight_layout()
    os.chdir(path_save)
    plt.savefig( 'Re_green_tensor_self' + '.png', format='png')
...
if save_graphs==1:
    plt.tight_layout()
    plt.savefig( 'Im_green_tensor_self' + '.png', format='png')
...
if save_graphs==1:
    plt.tight_layout()
    os.chdir(path_save)
    plt.savefig( 'Re_green_tensorQE_self' + '.png', format='png')
...
if save_graphs==1:
    plt.tight_layout()
    plt.savefig( 'Im_green_tensorQE_self' + '.png', format='png')
