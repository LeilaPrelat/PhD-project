...
#    Int05_A_function_im = lambda w,u: np.imag(J0(w,u)*u*expA(u)*exp_electron(w)*(alpha_z1(u) + 1/alpha_z1(u)))
    ...
    int05A_re,err = integrate.dblquad(Int05_A_function_re, 0, cota_sup1, lambda u: 0, lambda u:cota_sup2A)
#    int05A_im,err = integrate.dblquad(Int05_A_function_im, 0, cota_sup1, lambda u: 0, lambda u:cota_sup2)

###################### parte B ###############################################

    Int05_B_function_re = lambda w,u: -np.real(J0(w,u)*u*expB(u)*sin_electron(w)*(alpha_z2(u) - 1/alpha_z2(u))) #hay un 1j afuera, por eso la parte real es con sin y con un menos

    Int05_B_function_im = lambda w,u: np.real(J0(w,u)*u*expB(u)*cos_electron(w)*(alpha_z2(u) - 1/alpha_z2(u)))
