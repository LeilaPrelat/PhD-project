...
plot_sin_numerical = 0 #graficar solamente soluciones analiticas 

# las soluciones numericas tienen muchos problemas para frecuencias/energia altas
...
path_save = path_basic + '/' + 'fieldE_ref2'
...
try:
    ...
    from fieldE_ref_numerical2 import Efield_NUM_QE,Efield_NUM,Efield_NUM_SP,Efield_ana1,Efield_ana2,Efield_ana1b
...
if plot_xy_3D ==1 :
    ...
    z0 = 1
    title1 = r'$\epsilon_1$ = %i, v = c/%i $\mu$m/s, $\omega$ = %.2fTHz, b = %.2fnm' %(epsi1,int_v,omegaTHz0,b*1e3) 
    label1 = '_b%i_3D' %(b)
    labelx,labely = 'x [$\mu$m]', 'y [$\mu$m]'
    title2A = r'$\epsilon_2$ = %i, $\hbar\mu$ = %.2feV, $\hbar\gamma$ = %.4feV, $z_p=$%.2fnm z = %i$\mu$m' %(epsi2,hbmu,hbgama,zp*1e3,z0) 
    title = title1 + '\n'  + title2A   
    ...
    list_E = np.array(np.linspace(0.05,1.05,21))*1e3
    list_OmegaTHz = np.linspace(10,410,101)
    
    #if plot_sin_numerical == 0 : # graficar numerico
    #    list_OmegaTHz = np.linspace(0.001,0.2,51)
    #    list_OmegaTHz = np.linspace(0.001,0.7,26)
       # list_OmegaTHz = np.linspace(0.01,2.01,51)
    #else:
    #    list_OmegaTHz = np.linspace(500,701,51)
    
    label1 = '_b%i_vsOmega' %(b)
    title1 = r'$\epsilon_1$ = %i, v = c/%i $\mu$m/s, x = %i$\mu$m, y = %i$\mu$m, b = %.2fnm' %(epsi1,int_v,x0,y0,b*1e3) 
    labelx = r'E [meV]'
    title2A = r'$\epsilon_2$ = %i, $\hbar\mu$ = %.2feV, $\hbar\gamma$ = %.4feV, $z_p$=%.2fnm, z = %i$\mu$m' %(epsi2,hbmu,hbgama,zp*1e3,z0) 
    title = title1 + '\n'  + title2A   
    ...
    label1 = '_b%i_vs_z' %(b)
    title1 = r'$\epsilon_1$ = %i, v = c/%i $\mu$m/s, x = %i$\mu$m, y = %i$\mu$m, b = %inm' %(epsi1,int_v,x0,y0,b*1e3) 
    labelx = r'z [$\mu$m]' 
...
if plot_xy_3D ==1 : 
    ...
    def functionQE_real(x,y):
        rta = Efield_NUM_QE(omegac0,epsi1,epsi2,hbmu,hbgama,int_v,b,zp,z0)
        return rta.real
    
    def function_real(x,y):
        rta = Efield_NUM(omegac0,epsi1,epsi2,hbmu,hbgama,int_v,b,zp,z0)
        return rta.real
...
#%%
    ...
    for omegaTHz in list_OmegaTHz:
        ...
        rta = Efield_ana1(omegac,epsi1,epsi2,hbmu,hbgama,int_v,b,zp,z0)
        listy_ana1_re.append(rta.real)
        listy_ana1_im.append(rta.imag)
    
        rta = Efield_ana2(omegac,epsi1,epsi2,hbmu,hbgama,int_v,b,zp,z0)
        listy_ana2_re.append(rta.real)
        listy_ana2_im.append(rta.imag)

        rta = Efield_ana1b(omegac,epsi1,epsi2,hbmu,hbgama,int_v,b,zp,z0)
        listy_ana1b_re.append(rta.real)
        listy_ana1b_im.append(rta.imag)
        
        if plot_sin_numerical == 0:
        
            rta = Efield_NUM_SP(omegac,epsi1,epsi2,hbmu,hbgama,int_v,b,zp,z0)
            listySP_re.append(rta.real)
            listySP_im.append(rta.imag)
    
            
            rta = Efield_NUM(omegac,epsi1,epsi2,hbmu,hbgama,int_v,b,zp,z0)
            listy_re.append(rta.real)
            listy_im.append(rta.imag)
        
            rta = Efield_NUM_QE(omegac,epsi1,epsi2,hbmu,hbgama,int_v,b,zp,z0)
...
#    plt.plot(list_E,listy_ana2_re,'--',ms = ms,color = 'green',label = 'analytical 2')
   # plt.yscale('log')
    ...
    if plot_sin_numerical == 0:
        ...
        plt.plot(list_E,listySP_im,'--',ms = ms,color = 'red',label = 'numerical SP')
        # plt.yscale('log')
        plt.legend(loc = 'best',markerscale=mk,fontsize=tamlegend,frameon=True,handletextpad=hp, handlelength=1)
        if save_graphs==1:
            plt.tight_layout()
    ...
    graph(title,labelx,r'Im$(E_{ref,x})$',tamfig,tamtitle,tamletra,tamnum,labelpadx,labelpady,pad)
    plt.plot(list_E,listy_ana1_im,'.',ms = ms,color = 'blue',label = 'analytical 1')
    plt.plot(list_E,listy_ana1b_im,'.',ms = ms,color = 'magenta',label = 'analytical 1b')
...
#%%
    ...
    for z in list_z:
        ...
        rta = Efield_NUM_SP(omegac0,epsi1,epsi2,hbmu,hbgama,int_v,b,zp,z)
        listySP_re.append(rta.real)
        listySP_im.append(rta.imag)
    
        rta = Efield_ana1(omegac0,epsi1,epsi2,hbmu,hbgama,int_v,b,zp,z)
        listy_ana1_re.append(rta.real)
        listy_ana1_im.append(rta.imag)
    
        rta = Efield_ana1b(omegac0,epsi1,epsi2,hbmu,hbgama,int_v,b,zp,z)
        listy_ana1b_re.append(rta.real)
        listy_ana1b_im.append(rta.imag)
    
        rta = Efield_ana2(omegac0,epsi1,epsi2,hbmu,hbgama,int_v,b,zp,z)
        listy_ana2_re.append(rta.real)
        listy_ana2_im.append(rta.imag)
        
        if plot_sin_numerical == 0:
            rta = Efield_NUM(omegac0,epsi1,epsi2,hbmu,hbgama,int_v,b,zp,z)
            listy_re.append(rta.real)
            listy_im.append(rta.imag)
        
            rta = Efield_NUM_QE(omegac0,epsi1,epsi2,hbmu,hbgama,int_v,b,zp,z)
            listyQE_re.append(rta.real)
            listyQE_im.append(rta.imag)
    ...
    graph(title,labelx,r'Re$(E_{ref,x})$',tamfig,tamtitle,tamletra,tamnum,labelpadx,labelpady,pad)
    plt.plot(list_z,listySP_re,'--',ms = ms,color = 'red',label = 'numerical SP')
    plt.plot(list_z,listy_ana1_re,'.',ms = ms,color = 'blue',label = 'analytical 1')
    plt.plot(list_z,listy_ana1b_re,'.',ms = ms,color = 'magenta',label = 'analytical 1b')
    plt.plot(list_z,listy_ana2_re,'--',ms = ms,color = 'black',label = 'analytical 2')
    plt.yscale('log')
    plt.legend(loc = 'best',markerscale=mk,fontsize=tamlegend,frameon=True,handletextpad=hp, handlelength=1)
    if save_graphs==1:
        plt.tight_layout()
    ...
    graph(title,labelx,r'Im$(E_{ref,x})$',tamfig,tamtitle,tamletra,tamnum,labelpadx,labelpady,pad)
    plt.plot(list_z,listySP_im,'-',ms = ms,color = 'red',label = 'numerical SP')
    plt.plot(list_z,listy_ana1_im,'.',ms = ms,color = 'blue',label = 'analytical 1')
    plt.plot(list_z,listy_ana1b_im,'.',ms = ms,color = 'magenta',label = 'analytical 1b')
    plt.plot(list_z,listy_ana2_im,'--',ms = ms,color = 'black',label = 'analytical 2')
    plt.legend(loc = 'best',markerscale=mk,fontsize=tamlegend,frameon=True,handletextpad=hp, handlelength=1)
    if save_graphs==1:
        plt.tight_layout()
        plt.savefig( 'Im_Eref2_' + label1 + '.png', format='png')    
