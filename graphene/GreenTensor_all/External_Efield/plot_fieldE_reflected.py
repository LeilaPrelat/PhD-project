...
plot_sin_numerical =  1 #graficar solamente soluciones analiticas 
...
if plot_xy_3D ==1 :
    ...
    title2A = r'$\epsilon_2$ = %i, $\hbar\mu$ = %.2feV, $\hbar\gamma$ = %.4feV, $z_p=$%.2fnm z = %inm' %(epsi2,hbmu,hbgama,zp*1e3,z0*1e3) 
    title = title1 + '\n'  + title2A   
    ...
    list_z = np.linspace(b-0.01,zp+0.01,65)
    label1 = '_b%i_vs_z' %(b)
...
if plot_xy_3D ==1 : 
    ...
    def functionQE_real(x,y):
        rta = Efield_NUM_QE(omegac0,epsi1,epsi2,hbmu,hbgama,int_v,b,zp,x,y,z0)
        return rta.real
    
    def function_real(x,y):
        rta = Efield_NUM(omegac0,epsi1,epsi2,hbmu,hbgama,int_v,b,zp,x,y,z0)
        return rta.real
...
#%%
    ...
    for omegaTHz in list_OmegaTHz:
        ...
        if plot_sin_numerical == 0:
            ...
            rta = Efield_NUM(omegac,epsi1,epsi2,hbmu,hbgama,int_v,b,zp,x0,y0,z0)
            listy_re.append(rta.real)
            listy_im.append(rta.imag)
        
            rta = Efield_NUM_QE(omegac,epsi1,epsi2,hbmu,hbgama,int_v,b,zp,x0,y0,z0)
...
#    plt.plot(list_E,listy_ana1b_re,'.',ms = ms,color = 'magenta',label = 'analytical 1b')
   ...
   # plt.yscale('log')
    ...
    if save_graphs==1:
        plt.tight_layout()
        os.chdir(path_save)
        plt.savefig( 'Re_Eref3_' + label1 + '.png', format='png')
...
#    plt.yscale('log')
    ...
    if save_graphs==1:
        plt.tight_layout()
        plt.savefig( 'Im_Eref3_' + label1 + '.png', format='png')      
...
#%%
    ...
    for z in list_z:
        ...
        if plot_sin_numerical == 0:
            ...
            rta = Efield_NUM(omegac0,epsi1,epsi2,hbmu,hbgama,int_v,b,zp,x0,y0,z)
            listy_re.append(rta.real)
            listy_im.append(rta.imag)
        
            rta = Efield_NUM_QE(omegac0,epsi1,epsi2,hbmu,hbgama,int_v,b,zp,x0,y0,z)
            listyQE_re.append(rta.real)
            listyQE_im.append(rta.imag)
