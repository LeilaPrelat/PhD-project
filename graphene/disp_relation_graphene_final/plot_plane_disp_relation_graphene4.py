...
list_omega_THz = np.linspace(0.001,70,n)
...
list_omegaTHz = np.array(list_mEv)*1e-3*1e-12/(hb)
omegaD_THz = 30
valuey_omegaD_THz = k_parallel(omegaD_THz,mu_c,gamma_in,epsi1,epsi2)

plt.figure(figsize=tamfig)    
#plt.title(title1 + ', ' + title2 + '\n',fontsize=tamtitle)
plt.plot(list_y_re,list_omegaTHz,'-',color = 'darkgreen',lw = ms)
plt.plot(valuey_omegaD_THz,omegaD_THz,'.',ms = 2,color = 'blue')
...
#        omegaTHz = omega*1e12        
    ...
    j = j + 1
plt.plot(omegaD_THz*1e12/v,omegaD_THz,'.',ms = 2,color = 'blue')
...
plt.tight_layout()
# os.chdir(path_save)
plt.savefig('disp_relation_graphene_imag_vs_mEv_2.png')
