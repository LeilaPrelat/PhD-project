...
try:
    sys.path.insert(1, path_basic)
    from green_tensor_direct import green_tensor_ANA_QE
except ModuleNotFoundError:
...
try:
    sys.path.insert(1, path_basic)
    from green_tensor_ref_xx_graphene import green_tensor_PP2, green_tensor_ref_pole_aprox
except ModuleNotFoundError:
...
#
#def green_self_num_mix(omegac,epsi1,epsi2,hbmu,hbgama,zp) :
#    
#    real_part = np.real(green_self_num(omegac,epsi1,epsi2,hbmu,hbgama,zp))
#    imaginary_part = np.imag(green_self_ana2(omegac,epsi1,epsi2,hbmu,hbgama,zp))
#    
...
def dipole_moment_x_ana(omegac,epsi1,epsi2,hbmu,hbgama,int_v,b,zp,x1,z1,x2,z2,omega01,kappa_factor_omega01,kappa_r_factor1,omega02,kappa_factor_omega02,kappa_r_factor2):     
    ...
    cte_aux = 1 - alffa_eff1_x*alffa_eff2_x*(Gdir_12 + Gref_12)**2
    px = alffa_eff1_x*( Edir1 + Eref1 + alffa_eff2_x*(Gdir_12 + Gref_12)*(Edir2 + Eref2) )/cte_aux
...
def dipole_moment_x_ana_2(omegac,epsi1,epsi2,hbmu,hbgama,int_v,b,zp,x1,z1,x2,z2,omega01,kappa_factor_omega01,kappa_r_factor1,omega02,kappa_factor_omega02,kappa_r_factor2):     
    """    
    Parameters
    ----------
    omegac : omega/c = k0 en 1/micrometros    
    ...
    cte_aux = 1 - alffa_eff1_x*alffa_eff2_x*(Gdir_12 + Gref_12)**2
    px = alffa_eff2_x*( Edir2 + Eref2 + alffa_eff1_x*(Gdir_12 + Gref_12)*(Edir1 + Eref1) )/cte_aux
