...
def Ex_many_dipoles_num(omegac,epsi1,epsi2,hbmu,hbgama,x,y,z,a,N,zp,int_v,px,py,pz):
    """
    Parameters
    ----------
    ...
    int_tot0_x = lambda ky: 0
    int_tot0_y = lambda ky: 0
    for n in range(N):
        print(n)
    ...
    ############ I1 ###########################################################
        int1_re = lambda ky: np.real(1j*rp(ky)*(px*kx + ky*py)*exp_z(ky)*exp_xy(ky)/k_parallel(ky))
        int1_im = lambda ky: np.imag(1j*rp(ky)*(px*kx + ky*py)*exp_z(ky)*exp_xy(ky)/k_parallel(ky))
    
    ################### I2 ###############################################
        int2_re = lambda ky: np.real(-rp(ky)*pz*np.sign(z)*exp_z(ky)*ky*exp_xy(ky))
        int2_im = lambda ky: np.imag(-rp(ky)*pz*np.sign(z)*exp_z(ky)*ky*exp_xy(ky))
...
#        int3_im,err = integrate.quad(Int3_B_function_im, cota_inf, cota_sup)
    ################### I4 ###############################################
        int3_re = lambda ky: np.real(-(px*kx + py*ky)*1j*exp_z(ky)*exp_xy(ky)/k_parallel(ky))
        int3_im = lambda ky: np.imag(-(px*kx + py*ky)*1j*exp_z(ky)*exp_xy(ky)/k_parallel(ky))
    
    ################### I5 ###############################################
        int4_re = lambda ky: np.real(pz*np.sign(z)*exp_z(ky)*ky*rp(ky)*exp_xy(ky))
        int4_im = lambda ky: np.imag(pz*np.sign(z)*exp_z(ky)*ky*rp(ky)*exp_xy(ky))
...
#        int6_im,err = integrate.quad(Int6_B_function_im, cota_inf, cota_sup)
    ################### I6 ###############################################    
        ...
        int_re_tot = lambda ky: int1_re(ky) + int2_re(ky) + int3_re(ky) + int4_re(ky) 
        int_im_tot = lambda ky: int1_im(ky) + int2_im(ky) + int3_im(ky) + int4_im(ky)
        
        int_tot0_x = lambda ky: int_tot0(ky) - a*1j*kx*(int_re_tot(ky) + 1j*int_im_tot(ky))
