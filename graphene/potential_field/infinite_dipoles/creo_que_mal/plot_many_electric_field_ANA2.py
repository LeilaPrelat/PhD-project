...
path_save = path_basic + '/' + 'many_electric_field_ANA2'
...
try:
    ...
    from electric_field_many_dipoles2 import phi_many_dipoles_ana2
...
if plot_vs_xy == 1:
    ...
    z0 = zp
    label1 = r'_xy_3D_ana' + labelp  + '_E%.4f' %(E0)
    ...
    if plot_Etot == 1:
        def function_Etot(x_nano,y_nano):
            ...
            Ex,Ey,Ez = phi_many_dipoles_ana2(omegac0,epsi1,epsi2,hbmu,hbgama,x,y,z0,a,N,zp,int_v,px,py,pz)
...
#            rta = np.abs(Ex)**2 + np.abs(Ey)**2 + np.abs(Ez)**2
    ...
    def function_Etot(energy0):
        omegac0 = energy0/aux           
        rta = phi_many_dipoles_ana2(omegac0,epsi1,epsi2,hbmu,hbgama,x0,y0,z0,a,N,zp,int_v,px,py,pz)
...
if plot_Etot == 1:
    ...
    f1_imag = np.vectorize(function_Ex_imag)
    Z1_imag = f1_real(X, Y)
    
    graph(title,labelx,labely,tamfig,tamtitle,tamletra,tamnum,labelpadx,labelpady,pad)
    im = plt.imshow(Z1_imag, extent = limits, cmap='RdBu_r', interpolation='bilinear')
    cbar = plt.colorbar(im)
    cbar.ax.tick_params(labelsize = tamnum)
...
if periodicity == 1:
    ...
    def function_Etot1D(x_nano):
        ...
        rta = phi_many_dipoles_ana2(omegac0,epsi1,epsi2,hbmu,hbgama,x,y,z0,a,N,zp,int_v,px,py,pz)
        return rta
