...
#        print(n)
        ...
        exp_z2 = lambda ky: np.exp(-2*k_parallel(ky)*zp)
    ...
    ################### I2 ###############################################
        Int2_B_function_re = lambda ky: np.real(pz*ky*np.sign(z)*exp_z1(ky)*exp_xy(ky))
        Int2_B_function_im = lambda ky: np.imag(pz*ky*np.sign(z)*exp_z1(ky)*exp_xy(ky))
    ...
    ################### I3 ###############################################
        Int3_B_function_re = lambda ky: np.real(px*kx*1j*exp_z1(ky)*rp(ky)*exp_xy(ky)*exp_z2(ky)/k_parallel(ky))
        Int3_B_function_im = lambda ky: np.imag(px*kx*1j*exp_z1(ky)*rp(ky)*exp_xy(ky)*exp_z2(ky)/k_parallel(ky))
    ...
    ################### I4 ###############################################
        Int4_B_function_re = lambda ky: np.real(py*ky*1j*exp_z1(ky)*ky*rp(ky)*exp_xy(ky)*exp_z2(ky)/k_parallel(ky)) ##este termino estaba mal
        Int4_B_function_im = lambda ky: np.imag(py*ky*1j*exp_z1(ky)*ky*rp(ky)*exp_xy(ky)*exp_z2(ky)/k_parallel(ky))##este termino estaba mal
#    
    ...
    ################### I5 ###############################################
        Int5_B_function_re = lambda ky: np.real(-pz*ky*np.sign(z)*exp_z1(ky)*rp(ky)*exp_xy(ky)*exp_z2(ky))
        Int5_B_function_im = lambda ky: np.imag(-pz*ky*np.sign(z)*exp_z1(ky)*rp(ky)*exp_xy(ky)*exp_z2(ky))
...
def phi_many_dipoles_ana(omegac,epsi1,epsi2,hbmu,hbgama,x,y,z,a,N,zp,int_v,px,py,pz):
    """
    Parameters
    ----------
    omegac : omega/c = k0 en 1/micrometros    
