...
path_save = path_basic + '/' + 'term2_phi_integrand'
...
try:
    ...
    from term_integrand_phi_many_dipole import phi_many_dipoles_integrand_term2
...
def function_num_re(energy0,alpha_y):
    ...
    rta = phi_many_dipoles_integrand_term2(omegac0,epsi1,epsi2,hbmu,hbgama,x,y,a,Ndip,zp,int_v,px,alpha_y)
...
#%%
N = 100
list_x = np.linspace(0.001,100,N)
list_y = np.linspace(0.001,100,N)
#energy0 = 43
#omega0THz = energy0*1e-12/hb
#omegac0 = energy0/(c*hb)
#
...
plt.ylabel(r'|M$_2$($\alpha_{y}$,k)|',fontsize=tamletra, labelpad = 0)
...
plt.tight_layout()
os.chdir(path_save)
plt.savefig('integrandM2_E0%i.png' %(energy0))
