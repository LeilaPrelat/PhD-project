...
path_save = path_basic + '/' + 'term3_phi_integrand'
...
try:
    ...
    from term_integrand_phi_many_dipole import phi_many_dipoles_integrand_term3
...
alfa_x = int_v + 2*pi*Ndip/(omegac0*a)
...
def function_num_re(energy0,alpha_y):
    omegac0 = energy0*1e-3/aux 

    rta = phi_many_dipoles_integrand_term3(omegac0,epsi1,epsi2,hbmu,hbgama,x,y,a,Ndip,zp,int_v,pz,alpha_y)
    
    return np.real(rta)

def function_num_im(energy0,alpha_y):
    omegac0 = energy0*1e-3/aux 
...
def function_num_abs(energy0,alpha_y):
    omegac0 = energy0*1e-3/aux 
...
plt.ylabel(r'|M$_3$($\alpha_{y}$,k)|',fontsize=tamletra, labelpad = 0)
...
plt.tight_layout()
os.chdir(path_save)
plt.savefig('integrandM3_E0%i.png' %(energy0))
