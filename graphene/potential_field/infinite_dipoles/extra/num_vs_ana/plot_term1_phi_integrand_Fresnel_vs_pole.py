...
path_save = path_basic + '/' + 'integrand_Fresnel_rp'
...
try:
    ...
    from term_integrand_phi_many_dipole_fresnel_vs_pole_aprox import phi_many_dipoles_integrand_term1_rp,phi_many_dipoles_integrand_term1_fresnel
...
title3 = r'pz = %i, a = %inm, N = %i, $z_p$=%inm, x=%inm, y=%inm' %(pz,a*1e3, Ndip*2+1, zp*1e3,x*1e3,y*1e3)
...
def function_fresnel_re(energy0,alpha_y):
    omegac0 = energy0*1e-3/aux 

    rta = phi_many_dipoles_integrand_term1_fresnel(omegac0,epsi1,epsi2,hbmu,hbgama,x,y,a,Ndip,zp,int_v,pz,alpha_y)
    
    return np.real(rta)

def function_fresnel_im(energy0,alpha_y):
    omegac0 = energy0*1e-3/aux 
...
def function_fresnel_abs(energy0,alpha_y):
    omegac0 = energy0*1e-3/aux 
...
def function_rp_re(energy0,alpha_y):
    omegac0 = energy0*1e-3/aux 

    rta = phi_many_dipoles_integrand_term1_rp(omegac0,epsi1,epsi2,hbmu,hbgama,x,y,a,Ndip,zp,int_v,pz,alpha_y)
    
    return np.real(rta)

def function_rp_im(energy0,alpha_y):
    omegac0 = energy0*1e-3/aux 
...
def function_rp_abs(energy0,alpha_y):
    omegac0 = energy0*1e-3/aux 
...
#    energy0 = 2*alfac*hbmu*u
    ...
    rta = function_fresnel_abs(energy0,u)
    list_y1D_fresnel.append(rta)

    rta = function_rp_abs(energy0,u)
...
plt.tight_layout()
os.chdir(path_save)
plt.savefig('integrandM_E0%i.png' %(energy0))
