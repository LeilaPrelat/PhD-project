...
path_save = path_basic + '/' + 'electric_field_x'
...
try:
    ...
    from electric_field_inf_dipoles import Ex_many_dipoles_num2,Ex_many_dipoles_ana2
...
title1 = r'v = c/%i $\mu$m/s, b = %inm, E = %.2feV, a = %inm' %(int_v,b*1e3,E0,a*1e3) 
...
if plot_xy_3D == 1 :    #omega = 0.7*1e12
    ...
    def function_real_num(x_nano,y_nano):
        ...
        rta = Ex_many_dipoles_num2(omegac0,epsi1,epsi2,hbmu,hbgama,x,y,z0,a,zp,int_v,px,py,pz)
        return rta.real
    ...
    def function_real_ana(x_nano,y_nano):
        ...
        rta = Ex_many_dipoles_ana2(omegac0,epsi1,epsi2,hbmu,hbgama,x,y,z0,a,zp,int_v,px,py,pz)
        return rta.real
    ...
    def function_real_num(y_nano,z_nano):
        y = y_nano*1e-3 
        z = z_nano*1e-3        
        
        rta = Ex_many_dipoles_num2(omegac0,epsi1,epsi2,hbmu,hbgama,x0,y,z,b,zp,px,py,pz)
        return rta.real

    def function_imag_num(y_nano,z_nano):
        y = y_nano*1e-3 
        z = z_nano*1e-3        
        
        rta = electric_field_function(omegac0,epsi1,epsi2,hbmu,hbgama,x0,y,z,b,zp,px,py,pz)
        return rta.imag
    ...
    def function_real_num(x_nano,z_nano):
        x = x_nano*1e-3 
        z = z_nano*1e-3
        
        rta = electric_field_function(omegac0,epsi1,epsi2,hbmu,hbgama,x,y0,z,b,zp,px,py,pz)
        return rta.real

    def function_imag_num(x_nano,z_nano):
        x = x_nano*1e-3 
        z = z_nano*1e-3
...
if plot_vs_Energy == 0:
    ...
    if plot_xy_3D == 1:
        plt.plot((xD*1e3)*aux_list, list_y,'--',color = 'green')
        plt.plot(list_x, (yD*1e3)*aux_list,'--',color = 'green')
    elif plot_yz_3D == 1:    
        plt.plot((yD*1e3)*aux_list, list_y,'--',color = 'green')
        plt.plot(list_x, (zD*1e3)*aux_list,'--',color = 'green')
    elif plot_xz_3D == 1:    
        plt.plot((xD*1e3)*aux_list, list_y,'--',color = 'green')
        plt.plot(list_x, (zD*1e3)*aux_list,'--',color = 'green')
    ...
    if vmin < 0 :
          tick_locations = ( [-(10.0**x) for x in np.linspace(minlog,-1,minlog + 2)] 
                            + [0] 
                            + [(10.0**x) for x in np.linspace(-1,maxlog, minlog + 2)] )
    else:
          tick_locations = ( [(10.0**x) for x in np.linspace(minlog,maxlog,maxlog - minlog + 1) ])    
...
#    im = plt.imshow(Z1_real, extent = limits, cmap=plt.cm.hot, interpolation='bilinear')
    ...
    if save_graphs == True:
        os.chdir(path_save)
        plt.tight_layout()
        plt.savefig('Re_Efield_log' + label1 + '.png', format='png')
    ...
    if save_graphs == True:
        os.chdir(path_save)
        plt.tight_layout()
        plt.savefig('Re_Efield' + label1 + '.png', format='png')
    ...
    if save_graphs == True:
        os.chdir(path_save)
        plt.tight_layout()
        plt.savefig('Im_Efield' + label1 + '.png', format='png')        
