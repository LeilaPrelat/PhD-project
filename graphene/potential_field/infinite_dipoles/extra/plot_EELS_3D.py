...
def EELS(omegac,epsi1,epsi2,hbmu,hbgama,y,zp,int_v,px,py,pz,a,b):
    E = omegac*aux
    omega = omegac*c
...
def EELS_dir(omegac,epsi1,epsi2,hbmu,hbgama,y,zp,int_v,px,py,pz,a,b):
    E = omegac*aux
    omega = omegac*c
...
title2 = r'$z_p$=%inm, px=%i, py=%i, pz=%i, a = %inm' %(zp*1e3,px,py,pz,a*1e3)
...
if plot_vs_v == 1:
    ...
    def EELS_3D_ind(int_v_value,energy0):
        ...
        return EELS(omegac0,epsi1,epsi2,hbmu,hbgama,y0,zp,1/int_v_value,px,py,pz,a,b)

    def EELS_3D_dir(int_v_value,energy0):
        omegac0 = energy0*1e-3/aux     
        return EELS_dir(omegac0,epsi1,epsi2,hbmu,hbgama,y0,zp,1/int_v_value,px,py,pz,a,b)
    ...
    def EELS_3D_ind(b_value_nm,energy0):
        ...
        return EELS(omegac0,epsi1,epsi2,hbmu,hbgama,y0,zp,int_v,px,py,pz,a,b_value)


    def EELS_3D_dir(b_value_nm,energy0):
        omegac0 = energy0*1e-3/aux 
        b_value = b_value_nm*1e-3
        return EELS_dir(omegac0,epsi1,epsi2,hbmu,hbgama,y0,zp,int_v,px,py,pz,a,b_value)
   
    listx1 = np.linspace(-0.05,0.05,N)
    ...
    def EELS_3D_ind(y_value,energy0):
        omegac0 = energy0*1e-3/aux     
        return EELS(omegac0,epsi1,epsi2,hbmu,hbgama,y_value,zp,int_v,px,py,pz,a,b)

    def EELS_3D_dir(y_value,energy0):
        omegac0 = energy0*1e-3/aux     
        return EELS_dir(omegac0,epsi1,epsi2,hbmu,hbgama,y_value,zp,int_v,px,py,pz,a,b)
...
if plot_vs_y == 1:
    ...
    im = plt.imshow(Z_dir, extent = limits, cmap=plt.cm.hot, aspect=1.4) 
else:
...
if plot_vs_y == 1:
    ...
    im = plt.imshow(Z_ind, extent = limits, cmap=plt.cm.hot, aspect=1.4) 
else:
