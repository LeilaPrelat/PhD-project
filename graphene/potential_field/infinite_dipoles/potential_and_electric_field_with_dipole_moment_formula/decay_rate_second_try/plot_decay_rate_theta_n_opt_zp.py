...
def function_real_ana(zp_nano,energy0_meV,Nmax):
                
    omegac0 = energy0_meV*1e-3/(c*hb)  
    zp = zp_nano*1e-3
         
    rta = decay_rate_theta_inf_dipoles_ana_res(omegac0,epsi1,epsi2,hbmu,hbgama,int_v,zp,a,b,Nmax)
...
#plt.yscale('log')
os.chdir(path_save)
plt.savefig('decay_rate_' + label1 + '.png', format='png',bbox_inches='tight',pad_inches = 0.008,dpi = dpi)
