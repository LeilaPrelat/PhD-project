...
path_save = path_basic + '/' + 'electric_field_tot'
...
try:
    ...
    from electric_field_x_formula import electric_field_function_tot
...
title5 = r'xD = %inm, yD = %inm, zD = %inm' %(xD*1e3,yD*1e3,zD*1e3)
...
if plot_xy_3D == 1 :    #omega = 0.7*1e12
    ...
    def function_real(x_nano,y_nano):
        ...
        rta = electric_field_function_tot(omegac0,epsi1,epsi2,hbmu,hbgama,x,y,z0,xD,yD,zD,b,zp,px,py,pz)
        return rta
    ...
    def function_real(y_nano,z_nano):
        ...
        rta = electric_field_function_tot(omegac0,epsi1,epsi2,hbmu,hbgama,x0,y,z,xD,yD,zD,b,zp,px,py,pz)
        return rta
    ...
    def function_real(x_nano,z_nano):
        ...
        rta = electric_field_function_tot(omegac0,epsi1,epsi2,hbmu,hbgama,x,y0,z,xD,yD,zD,b,zp,px,py,pz)
        return rta
    ...
    def function_real(energy0):
        ...
        rta = electric_field_function_tot(omegac0,epsi1,epsi2,hbmu,hbgama,x0,y0,z0,xD,yD,zD,b,zp,px,py,pz)
        return rta    
...
#    im = plt.imshow(Z1_real, extent = limits, cmap=plt.cm.hot, interpolation='bilinear')
    ...
    cbar.set_label(r'$|E|^2$',fontsize=tamlegend,labelpad = 1)
    

    if save_graphs == True:
        os.chdir(path_save)
        plt.tight_layout()
        plt.savefig('Efield_tot_log' + label1 + '.png', format='png')
    ...
    if save_graphs == True:
        os.chdir(path_save)
        plt.tight_layout()
        plt.savefig('Efield_tot' + label1 + '.png', format='png')
