...
path_constants =  path_basic.replace('/potential_field/potential/potential_and_electric_field_simple_version/potential_1_dipolo_analytical_vs_num','')
path_save = path_basic + '/' + 'term_phi_integrand'
...
try:
    ...
    from term_phi_integrand import intergand_num
...
title3 = r'$z_p$=%inm, x=%inm, y=%inm' %(zp*1e3,x*1e3,y*1e3)
...
def function_num_re(energy0,u):
    ...
    rta = intergand_num(omegac0,epsi1,epsi2,hbmu,hbgama,x,y,zp,u)
...
plt.ylabel(r'|G$_2$($\alpha_{\parallel}$,k)|',fontsize=tamletra, labelpad = 0)
