...
def electric_potential_ana(omegac,epsi1,epsi2,hbmu,hbgama,x,y,z,zp,px,py,pz):     
    """    
    Parameters
    ----------
...
#    J1 = special.jv(1,kp*R)
    ...
    term1 = -term_px_py*((np.abs(z)**2/(term_aux**(3/2)) - 1/(term_aux**(1/2)))/R + Rp*np.pi*1j*kp_2*H1*exp_electron)

    term3 = -pz*np.sign(z)*(Rp*np.pi*1j*kp_2*H0*exp_electron + np.abs(z)/(term_aux**(3/2)) )
...
def electric_potential_num(omegac,epsi1,epsi2,hbmu,hbgama,x,y,z,zp,px,py,pz):     
    """    
    Parameters
    ----------
    omegac : omega/c = k0 en 1/micrometros    
...
#    k1_2 = (k0*cte1)**2
 #   n_v1 = int_v/cte1
    ...
    term2_final = -term_px_py*term2_int*k1_2
    ...
    term4_final = -pz*np.sign(z)*term4_int*k1_2
    
    return term1 + term3 + term2_final + term4_final 

#%%

def electric_potential_pole_aprox(omegac,epsi1,epsi2,hbmu,hbgama,x,y,z,zp,px,py,pz):     
    """    
    Parameters
    ----------
    omegac : omega/c = k0 en 1/micrometros    
