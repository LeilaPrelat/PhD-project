...
try:
    ...
    from potential2 import electric_potential_ana, electric_potential_num
...
if plot_vs_E == 1: 
    ...
    graph(title,labelx,'$|\phi_p|$',tamfig,tamtitle,tamletra,tamnum,labelpadx,labelpady,pad)
    plt.plot(listx,listy_abs_ana,'.',ms = ms*1.5,color = 'purple',label = 'pole aprox')
    plt.plot(listx,listy_abs_num,'.-',ms = ms,color = 'lightseagreen',alpha = 0.8,label = 'sin aprox')
    plt.legend(loc = 'best',markerscale=2,fontsize=tamlegend,frameon=0,handletextpad=0.2, handlelength=length_marker)
    plt.tight_layout()
    os.chdir(path_save)
