...
try:
    sys.path.insert(1, path_constants)
    from dipole_moment import dipole_moment_num, dipole_moment_anav2
except ModuleNotFoundError:
...
def electric_field_ana_f(omegac,epsi1,epsi2,hbmu,hbgama,int_v,px,py,pz,b,zp,phi,R,z,omega0,kappa_factor_omega0,kappa_r_factor):     
    """    
    Parameters
    ----------   
...
#    J1 = special.jv(1,kp*R)
    ...
    term_aux5 = term_aux**(5/2)
    term_aux3 = term_aux**(3/2)
    term_aux1 = term_aux**(1/2)
    
    term0 = - 3*np.abs(z)**2/term_aux5 + 1/term_aux3 - (np.abs(z)/R)**2/term_aux3 - (1/term_aux1)/(R**2) 
    
    term1 = term_px_py*( term0 + Rp*np.pi*1j*kp_3*0.5*(H0 - H2)*exp_electron)

    term3 = pz_v*np.sign(z)*(Rp*np.pi*1j*kp_3*0.5*(H_minus1 - H1)*exp_electron - 3*R*np.abs(z)/(term_aux**(5/2)))
