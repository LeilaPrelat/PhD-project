...
path_save = path_basic + '/' + 'decay_rate_dir'
if not os.path.exists(path_save):
    print('Creating folder to save graphs')
    os.mkdir(path_save)

err = 'decay_rate_dir.py no se encuentra en ' + path_basic
try:
    sys.path.insert(1, path_basic)
    from decay_rate_dir import EELS_dir_ana_f
...
def function_imag_ana(energy0,int_v,zp_nano):
    ...
    rta = EELS_dir_ana_f(omegac0,epsi1,epsi2,hbmu,hbgama,int_v,b,zp,omega0,kappa_factor_omega0,kappa_r_factor)
...
graph(title,labelx,'$\Gamma_{dir}$ [seg/nm$^2$]',tamfig,tamtitle,tamletra,tamnum,labelpadx,labelpady,pad)
