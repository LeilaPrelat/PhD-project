...
def potential_final_ana(omegac,epsi1,epsi2,hbmu,hbgama,int_v,b,zp,phi,R,z,omega0,kappa_factor_omega0,kappa_r_factor):
    ...
    px_py_term = px_v*np.cos(phi) + py_v*np.sin(phi)
    
    term1 = -px_py_term*(G2 + (np.abs(z)**2/term0_3 - 1/term0_1)/R)
    
    term2 = -pz_v*np.sign(z)*(G1 + np.abs(z)/term0_3)
...
def potential_final_pole_aprox(omegac,epsi1,epsi2,hbmu,hbgama,int_v,b,zp,phi,R,z,omega0,kappa_factor_omega0,kappa_r_factor):
...
def potential_final_num(omegac,epsi1,epsi2,hbmu,hbgama,int_v,b,zp,phi,R,z,omega0,kappa_factor_omega0,kappa_r_factor):
