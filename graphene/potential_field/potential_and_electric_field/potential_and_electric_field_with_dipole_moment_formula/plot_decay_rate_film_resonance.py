...
try:
    ...
    from decay_rate_film3_resonance import EELS_film_ana_f, EELS_parallel_ana_f,EELS_dir_ana_f
...
title4 = r'b = %i nm, L = %i $\mu$m' %(b*1e3,L_nano*1e-3)
labelp = r'_res' 
...
if plot_vs_E == 1: 
    ...
    for value in listx: 
        ...
        y_parallel_ana = function_parallel_ana(value,int_v0,zp0)        
        list_ana_parallel.append(y_parallel_ana)
...
#        print(value2)
        ...
        y_parallel_ana = function_parallel_ana(E0,value2,zp0)        
        list_ana_parallel.append(y_parallel_ana)
    ...
    for value in listx: 
        ...
        y_parallel_ana = function_parallel_ana(E0,int_v0,value)        
        list_ana_parallel.append(y_parallel_ana)        
...
graph(title,labelx,'$\Gamma_{SP}/\Gamma$',tamfig,tamtitle,tamletra,tamnum,labelpadx,labelpady,pad)
plt.plot(listx,listy_im_ana,'.-',ms = ms,color = 'purple',label = r'$\Gamma_{film}$')
...
#    plt.yscale('log')
os.chdir(path_save)
plt.savefig( 'EELS_film_' + label1 + '.png', format='png')   


graph(title,labelx,'$\Gamma$ [nano seg]',tamfig,tamtitle,tamletra,tamnum,labelpadx,labelpady,pad)
#plt.plot(listx,listy_im_ana,'.-',ms = ms,color = 'purple',label = r'$\Gamma_{film}$')
plt.plot(listx,list_ana_parallel,'.-',ms = ms,color = 'darkred',label = r'$\Gamma_{\parallel}$')
#plt.plot(np.ones(10)*maxi, listy_aux,'-k')
plt.legend(loc = 'best',markerscale=1.5,fontsize=tamlegend,frameon=0.2,handletextpad=0.2, handlelength=length_marker)
...
#    plt.yscale('log')
os.chdir(path_save)
plt.savefig( 'EELS_parallel_' + label1 + '.png', format='png')   