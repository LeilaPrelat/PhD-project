...
try:
    ...
    from EELS_1dip import EELS_ana_f_ind1,EELS_num_f,EELS_ana_f_dir,EELS_ana_f_ind2
...
def function_ana_re_ind1(energy0,int_v0):
...
def function_num_re(energy0,int_v0):
    ...
    rta = EELS_num_f(omegac0,epsi1,epsi2,hbmu,hbgama,int_v0,b,zp,omega0,kappa_factor_omega0,kappa_r_factor)
    
    return np.real(rta)


def function_ana_re_ind2(energy0,int_v0):
    omegac0 = energy0*1e-3/aux 

    rta = EELS_ana_f_ind2(omegac0,epsi1,epsi2,hbmu,hbgama,int_v0,b,zp,omega0,kappa_factor_omega0,kappa_r_factor)
...
if plot_vs_E == 1:
    for value in listx: 
        ...
        y_re_ana_ind1 = function_ana_re_ind1(value,int_v)  
        y_re_ana_ind2 = function_ana_re_ind2(value,int_v)  
        
        y_re_num1 = function_num_re(value,int_v)        
        
        y_re_ana_dir = function_ana_re_dir(value,int_v)        
        
        listy_re_ana_ind1.append(y_re_ana_ind1)
        listy_re_ana_ind2.append(y_re_ana_ind2)
        
        listy_re_num.append(y_re_num1)
    
        listy_re_ana_dir.append(y_re_ana_dir)
        listy_re_ana_tot.append(y_re_ana_dir + y_re_ana_ind2)
...
#        listy_im_num.append(y_im_num)
    ...
    for value in listx: 
        ...
        y_re_ana_ind1 = function_ana_re_ind1(E0,value2)        
        y_re_ana_ind2 = function_ana_re_ind2(E0,value2)  
...
plt.plot(listx,listy_re_ana_ind1,'.-',ms = ms,color = 'purple')
#plt.plot(listx,listy_re_num,'.-',ms = ms,color = 'lightseagreen',label ='full numerical' )
plt.tight_layout()
#if plot_vs_c == 1:
#    plt.yscale('log')
os.chdir(path_save)
plt.savefig( 'EELS_ind1' + label1 + '.png', format='png')   


graph(title,labelx,'$\Gamma_{ind}$',tamfig,tamtitle,tamletra,tamnum,labelpadx,labelpady,pad)
plt.plot(listx,listy_re_ana_ind2,'.-',ms = ms,color = 'purple')
#plt.plot(listx,listy_re_num,'.-',ms = ms,color = 'lightseagreen',label ='full numerical' )
plt.tight_layout()
#if plot_vs_c == 1:
#    plt.yscale('log')
os.chdir(path_save)
plt.savefig( 'EELS_ind2' + label1 + '.png', format='png')   
