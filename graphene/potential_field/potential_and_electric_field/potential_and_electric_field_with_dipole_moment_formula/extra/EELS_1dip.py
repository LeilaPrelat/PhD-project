...
#        print('opcion 2')
    ...
    rta = Rp*np.pi*int_v*omegac*kp_2*(pz_v*np.sign(pz_v)*aux0 + px_v*aux1  )*expo
    return rta/(np.pi*E)



def EELS_ana_f_ind2(omegac,epsi1,epsi2,hbmu,hbgama,int_v,b,zp,omega0,kappa_factor_omega0,kappa_r_factor):     
    """    
    Parameters
    ----------
    omegac : omega/c = k0 en 1/micrometros    
...
#    k0 = omegac #=omega/c
    ...
    if int_v < np.real(alfa_p):
        aux0 = (kp**2 - (omegac*int_v)**2)**(-1/2)
#        aux1 = np.cos(np.arcsin(omegac*int_v/kp))/aux0
        print('opcion 1', E*1e3)
#        print()

    else:
        aux0 = -1j*((omegac*int_v)**2 - kp**2 )**(-1/2)
        print('opcion 2', E*1e3)
#        aux1 = -kp/(aux0*(omegac*int_v +  aux0))
    
#        print('opcion 2')
    expo = np.exp(-kp*(-b + 2*zp))

    rta = -Rp*1j*np.pi*kp_2*aux0*(pz_v*np.sign(pz_v) + 1j*omegac*int_v*px_v/kp)*expo*1j*omegac*int_v
    return rta/(np.pi*E)
...
def EELS_num_f(omegac,epsi1,epsi2,hbmu,hbgama,int_v,b,zp,omega0,kappa_factor_omega0,kappa_r_factor):     
    """    
    Parameters
    ----------
    omegac : omega/c = k0 en 1/micrometros    
...
#    alfa_p = np.real(alfa_p)
    ...
    return 2*Rp*int_v*kp_3*(pz_v*np.sign(pz_v)*aux0 + px_v*aux1  )*expo
