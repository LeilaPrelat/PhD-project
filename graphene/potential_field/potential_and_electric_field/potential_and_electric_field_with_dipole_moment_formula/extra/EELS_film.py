...
#    J1 = special.jv(1,kp*R)
    ...
    term_R = Rp*np.pi*1j*kp_3*0.5*(-pz_v*np.sign(z)*(H_minus1 - H1) + term_px_py*(H0 - H2))*exp_electron
      
    
    term_z = z*Rp*np.pi*1j*kp_3*(-pz_v*np.sign(z)*H0 + term_px_py*H1)*exp_electron
#    print(pz_v*np.sign(z)*(H_minus1 - H1))
#    
#    print( term_px_py*(H0 - H2))
#    
    p_parallel = np.sqrt(px_v**2 + py_v**2)
    
    term_f2 = term_R*np.conjugate(p_parallel) + term_z*np.conjugate(pz_v)
###################################################################################################

    return term_f2 + 1j*1.5
