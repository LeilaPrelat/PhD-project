
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
@author: leila

graficar el campo externo directo con la convencion de z hacia abajo
en z = 0
graficar mapa de color x,y
"""

import numpy as np
import sys
import os 
import matplotlib.pyplot as plt
import seaborn as sns
sns.set()

#%%

plot_vs_R = 0
plot_vs_E = 1
plot_pole_aprox = 1

#%%

name_this_py = os.path.basename(__file__)
path = os.path.abspath(__file__) #path absoluto del .py actual
path_basic = path.replace('/' + name_this_py,'')
path_ctes =  path_basic.replace('/' + 'principal_value','')
path_save = path_basic + '/' + 'E_polp_v5_PV'
#if not os.path.exists(path_save):
#    print('Creating folder to save graphs')
#    os.mkdir(path_save)

err = 'E_PV_polp_v5.py no se encuentra en ' + path_basic
try:
    sys.path.insert(1, path_basic)
    from E_PV_polp_v5 import Ex_polp_pole_aprox,Ey_polp_pole_aprox, Ez_polp_pole_aprox
except ModuleNotFoundError:
    print(err)

try:
    sys.path.insert(1, path_ctes)
    from constants import constantes
except ModuleNotFoundError:
    print('constants.py no se encuentra en ' + path_ctes)

pi,hb,c,alfac,mu1,mu2,mu3 = constantes()
aux = c*hb
aux2 = 1e12/c

#%%

print('Definir parametros del problema')

#v = c/int_v
#omega = 0.7*1e12
cota = 75 #nanometros

epsilon2 = 12
d = 20*1e-3
zp = 0.05
z0 = zp
cte_phi0 = 8
phi0 = np.pi/cte_phi0

px,py,pz = 1,0,0

title0 = r'px = %i, py = %i, pz = %i, $\varphi$=$\pi$/%i' %(px,py,pz,cte_phi0)
title1 = r'z = %inm, $z_p$=%inm, $\epsilon_2$ = %i, d = %inm' %(z0*1e3,zp*1e3,epsilon2,d*1e3)
labelp = r'_d%inm_px%i_py%i_pz%i_phi%i' %(d*1e3,px,py,pz,phi0)
title = title1 + '\n' + title0

N = 50
if plot_vs_R == 1: 
    omega_omegaWG0 = 2.8 # meV 
    labelx = 'R [nm]'  
    title = title + ', ' + r'$\omega/\omega_{WG}$ = %.2f' %(omega_omegaWG0)
    label1 = 'vs_R' + labelp + '_omega_omegaWG%i' %(omega_omegaWG0)
    listx = np.linspace(50,800,N)
else:
    R0 = 100 #nanos
    # z0 = 0.06*1e3
    labelx = '$\omega/\omega_{WG}$'   
    title = title + ', ' + 'R = %inm' %(R0)
    label1 = 'vs_omega_omega_WG' + labelp
    listx = np.linspace(0.01,4,N)

#omegaWG = (np.pi/d)/(np.sqrt(epsilon2-1)) # el c esta metido en aux

#%%
def Ex_real_pole_aprox(R_nano,omega_omegaWG):
#    omega0 = energy0*1e-3/aux 
#    omega_omegaWG = omega0/omegaWG
    R = R_nano*1e-3

    rta = Ex_polp_pole_aprox(omega_omegaWG,d,R,phi0,z0,zp,px,py,pz)
    
    return rta.real

def Ex_imag_pole_aprox(R_nano,omega_omegaWG):
#    omega0 = energy0*1e-3/aux 
#    omega_omegaWG = omega0/omegaWG
    R = R_nano*1e-3
    
    rta = Ex_polp_pole_aprox(omega_omegaWG,d,R,phi0,z0,zp,px,py,pz)
    return rta.imag


def Ey_real_pole_aprox(R_nano,omega_omegaWG):
#    omega0 = energy0*1e-3/aux 
#    omega_omegaWG = omega0/omegaWG
    R = R_nano*1e-3

    rta = Ey_polp_pole_aprox(omega_omegaWG,d,R,phi0,z0,zp,px,py,pz)
    
    return rta.real

def Ey_imag_pole_aprox(R_nano,omega_omegaWG):
#    omega0 = energy0*1e-3/aux 
#    omega_omegaWG = omega0/omegaWG
    R = R_nano*1e-3
    
    rta = Ey_polp_pole_aprox(omega_omegaWG,d,R,phi0,z0,zp,px,py,pz)
    return rta.imag

def Ez_real_pole_aprox(R_nano,omega_omegaWG):
#    omega0 = energy0*1e-3/aux 
#    omega_omegaWG = omega0/omegaWG
    R = R_nano*1e-3

    rta = Ez_polp_pole_aprox(omega_omegaWG,d,R,phi0,z0,zp,px,py,pz)
    
    return rta.real

def Ez_imag_pole_aprox(R_nano,omega_omegaWG):
#    omega0 = energy0*1e-3/aux 
#    omega_omegaWG = omega0/omegaWG
    R = R_nano*1e-3
    
    rta = Ez_polp_pole_aprox(omega_omegaWG,d,R,phi0,z0,zp,px,py,pz)
    return rta.imag

#%%
    
tamfig = (4.5,3.5)
tamlegend = 12
tamletra = 12
tamtitle = 11
tamnum = 10
labelpady = -1.5
labelpadx = 2
pad = 0
mk = 2
ms = 4
hp = 0.3
length_marker = 1

def graph(title,labelx,labely,tamfig,tamtitle,tamletra,tamnum,labelpadx,labelpady,pad):
    plt.figure(figsize=tamfig)
    plt.xlabel(labelx,fontsize=tamletra,labelpad =labelpadx)
    plt.ylabel(labely,fontsize=tamletra,labelpad =labelpadx)
    plt.tick_params(labelsize = tamnum, pad = pad)
    plt.title(title,fontsize=int(tamtitle*0.9))

    return   

#%%

if plot_vs_R == 1: 
    

    listy_re_pole_aproxG1 = []
    listy_im_pole_aproxG1 = []
   

    listy_re_pole_aproxG2 = []
    listy_im_pole_aproxG2 = []

   
    listy_re_pole_aproxG3 = []
    listy_im_pole_aproxG3 = []
      
    j = 0 
    for value in listx: 

   
        y_re_pole_aproxG1 = Ex_real_pole_aprox(value,omega_omegaWG0)
        y_im_pole_aproxG1 = Ex_imag_pole_aprox(value,omega_omegaWG0)  
        
        listy_re_pole_aproxG1.append(y_re_pole_aproxG1)
        listy_im_pole_aproxG1.append(y_im_pole_aproxG1)   
                       
        y_re_pole_aproxG2 = Ey_real_pole_aprox(value,omega_omegaWG0)
        y_im_pole_aproxG2 = Ey_imag_pole_aprox(value,omega_omegaWG0)  
        
        listy_re_pole_aproxG2.append(y_re_pole_aproxG2)
        listy_im_pole_aproxG2.append(y_im_pole_aproxG2)   

       
        y_re_pole_aproxG3 = Ez_real_pole_aprox(value,omega_omegaWG0)
        y_im_pole_aproxG3 = Ez_imag_pole_aprox(value,omega_omegaWG0)  
        
        
        listy_re_pole_aproxG3.append(y_re_pole_aproxG3)
        listy_im_pole_aproxG3.append(y_im_pole_aproxG3)           

       
        print(j)
        
        j = j + 1

if plot_vs_E == 1: 

    listy_re_pole_aproxG1 = []
    listy_im_pole_aproxG1 = []
   
   
    listy_re_pole_aproxG2 = []
    listy_im_pole_aproxG2 = []
   
    listy_re_pole_aproxG3 = []
    listy_im_pole_aproxG3 = []
    
    j = 0
    for value in listx: 

        
        y_re_pole_aproxG1 = Ex_real_pole_aprox(R0,value)
        y_im_pole_aproxG1 = Ex_imag_pole_aprox(R0,value)  
        
        listy_re_pole_aproxG1.append(y_re_pole_aproxG1)
        listy_im_pole_aproxG1.append(y_im_pole_aproxG1)           
              
        y_re_pole_aproxG2 = Ey_real_pole_aprox(R0,value)
        y_im_pole_aproxG2 = Ey_imag_pole_aprox(R0,value)  
        
        listy_re_pole_aproxG2.append(y_re_pole_aproxG2)
        listy_im_pole_aproxG2.append(y_im_pole_aproxG2)          
        
        y_re_pole_aproxG3 = Ez_real_pole_aprox(R0,value)
        y_im_pole_aproxG3 = Ez_imag_pole_aprox(R0,value)  
        
        
        listy_re_pole_aproxG3.append(y_re_pole_aproxG3)
        listy_im_pole_aproxG3.append(y_im_pole_aproxG3)    
        
 #%%
   
graph(title,labelx,'PV Re($E_x$)',tamfig,tamtitle,tamletra,tamnum,labelpadx,labelpady,pad)
plt.plot(listx,listy_re_pole_aproxG1,'.-',ms = ms,color = 'darkred',label = 'PP numerical')
plt.legend(loc = 'best',markerscale=mk,fontsize=tamlegend,frameon=0,handletextpad=0.2, handlelength=length_marker)
plt.tight_layout()
os.chdir(path_save)
plt.savefig( 'Re_Ex_PV' + label1 + '.png', format='png')   

graph(title,labelx,'PV Im($E_x$)',tamfig,tamtitle,tamletra,tamnum,labelpadx,labelpady,pad)
plt.plot(listx,listy_im_pole_aproxG1,'.-',ms = ms,color = 'darkred',label = 'PP numerical')
plt.legend(loc = 'best',markerscale=mk,fontsize=tamlegend,frameon=0,handletextpad=0.2, handlelength=length_marker)
plt.tight_layout()
plt.savefig( 'Im_Ex_PV' + label1 + '.png', format='png')   


graph(title,labelx,'PV Re($E_y$)',tamfig,tamtitle,tamletra,tamnum,labelpadx,labelpady,pad)
plt.plot(listx,listy_re_pole_aproxG2,'.-',ms = ms,color = 'darkred',label = 'PP numerical')
plt.legend(loc = 'best',markerscale=mk,fontsize=tamlegend,frameon=0,handletextpad=0.2, handlelength=length_marker)
plt.tight_layout()
plt.savefig( 'Re_Ey_PV' + label1 + '.png', format='png')   

graph(title,labelx,'PV Im($E_y$)',tamfig,tamtitle,tamletra,tamnum,labelpadx,labelpady,pad)
plt.plot(listx,listy_im_pole_aproxG2,'.-',ms = ms,color = 'darkred',label = 'PP numerical')
plt.legend(loc = 'best',markerscale=mk,fontsize=tamlegend,frameon=0,handletextpad=0.2, handlelength=length_marker)
plt.tight_layout()
plt.savefig( 'Im_Ey_PV' + label1 + '.png', format='png')   


graph(title,labelx,'PV Re($E_z$)',tamfig,tamtitle,tamletra,tamnum,labelpadx,labelpady,pad)
plt.plot(listx,listy_re_pole_aproxG3,'.-',ms = ms,color = 'darkred',label = 'PP numerical')
plt.legend(loc = 'best',markerscale=mk,fontsize=tamlegend,frameon=0,handletextpad=0.2, handlelength=length_marker)
plt.tight_layout()
plt.savefig( 'Re_Ez_PV' + label1 + '.png', format='png')   

graph(title,labelx,'PV Im($E_z$)',tamfig,tamtitle,tamletra,tamnum,labelpadx,labelpady,pad)
plt.plot(listx,listy_im_pole_aproxG3,'.-',ms = ms,color = 'darkred',label = 'PP numerical')
plt.legend(loc = 'best',markerscale=mk,fontsize=tamlegend,frameon=0,handletextpad=0.2, handlelength=length_marker)
plt.tight_layout()
plt.savefig( 'Im_Ez_PV' + label1 + '.png', format='png')   
    
#%%
