...
#    kmedium = k_parallel_medium(omega,epsilon2)
    ...
    for pole in list_poles:
        ...
        if pole - deltta > k1:
            ...
            inf_limits.append(k1 + 0.001) ## el integrando diverge en kz = 0 (k parallel = k)
...
#
def integration_limits2(omega_omegaWG,d):
...
#    kmedium = k_parallel_medium(omega,epsilon2)
    ...
    N = 500
    list_u = np.linspace(-np.sqrt(epsilon2)+0.001,-0.999,N)

    find_peaks_list = []
    for u in list_u:
        find_peaks_list.append(-np.real(rs(u)))
        

    peaks, _ = find_peaks(find_peaks_list, height=0)

    list_poles = list_u[peaks]
...
#    print(list_poles)
    ...
    for pole in list_poles:
        ...
        if pole - deltta > -np.sqrt(epsilon2)*k1:
            ...
            inf_limits.append(-np.sqrt(epsilon2)*k1) ## el integrando diverge en kz = 0 (k parallel = k)
        

        if pole + deltta < k1:
            sup_limits.append(pole + deltta)
        else:
            sup_limits.append(-k1 + 0.001)    
    
    return [inf_limits,sup_limits]

#%%

def Ex_pols_num_G1(omega_omegaWG,d,R,phi,z,zp,px,py,pz):     
    """    
    Parameters
    ----------
...
#
    ...
    termEx_num_int_re,err = integrate.quad(termEx_num_re_f, 0.001, 0.999, limit=limitt) 
    termEx_num_int_im,err = integrate.quad(termEx_num_im_f, 0.001, 0.999, limit=limitt) 
        
    return 1j*0.5*(termEx_num_int_re + 1j*termEx_num_int_im)*(k1**3)  

#%%

def Ex_pols_num_G2(omega_omegaWG,d,R,phi,z,zp,px,py,pz):     
    """    
    Parameters
    ----------
    omegac : omega/c = k0 en 1/micrometros    
...
def Ex_pols_num_G4(omega_omegaWG,d,R,phi,z,zp,px,py,pz):     
    """    
    Parameters
    ----------
    omegac : omega/c = k0 en 1/micrometros    
...
#
    ...
    termEx_num_int_re,err = integrate.quad(termEx_num_re_f, -0.999, 0.001, limit=limitt) 
    termEx_num_int_im,err = integrate.quad(termEx_num_im_f, -0.999, 0.001, limit=limitt) 
...
def Ex_pols_num_G5(omega_omegaWG,d,R,phi,z,zp,px,py,pz):     
    """    
    Parameters
    ----------
    omegac : omega/c = k0 en 1/micrometros    
...
#    termEx_num_int_re,err = integrate.quad(termEx_num_re_f, -np.sqrt(epsilon2) +0.001, -0.999, limit=limitt) 
#    termEx_num_int_im,err = integrate.quad(termEx_num_im_f, -np.sqrt(epsilon2) +0.001, -0.999, limit=limitt) 
#    
#    termEx_num_int = termEx_num_int_re + 1j*termEx_num_int_im
...
#    term0_re = term0_re + termEx_num_int        
    ...
    [inf_limits,sup_limits] = integration_limits2(omega_omegaWG,d)
    for j in range(len(sup_limits)):
...
def Ex_pols_pole_aproxG2(omega_omegaWG,d,R,phi,z,zp,px,py,pz):     
    """    
    Parameters
    ----------
    omegac : omega/c = k0 en 1/micrometros    
...
#        print(qj_polo)
        ...
        term0 = termEx_num_int_re + 1j*termEx_num_int_im + term0
