...
listx = np.linspace(0.01,np.sqrt(epsilon2),N)
listx = np.linspace(0.01,np.sqrt(epsilon2)-0.001,N)
listx = np.linspace(-100,100,N)


omegaWG = (np.pi/d)/(np.sqrt(epsilon2-1)) # el c esta metido en aux
k1 = omega_omegaWG0*omegaWG

list_zeros = [0]
arg = R0*k1
if px == 0 and phi0 == np.pi/4:

    list_zeros2 = []
    list_zeros = special.jn_zeros(2, 1000)/arg
    for j in range(len(list_zeros)):
        zeros_el =  list_zeros[j]
            
        if 0.01 < zeros_el < 1:
            
            list_zeros2.append(zeros_el)
