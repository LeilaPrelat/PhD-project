...
#    kmedium = k_parallel_medium(omega,epsilon2)
    ...
    for pole in list_poles:
        ...
        if pole - deltta > 1.05:
            ...
            inf_limits.append(1 + 0.05) ## el integrando diverge en kz = k
...
def Ex_polp_num(omega_omegaWG,d,R,phi,z,zp,px,py,pz):     
    ...
    for j in range(len(sup_limits)):
        ...
        termEx_num_int_re,err = integrate.quad(termEx_num_re_f, inf_limits_int, inf_limits_sup) 
        termEx_num_int_im,err = integrate.quad(termEx_num_im_f, inf_limits_int, inf_limits_sup) 
...
def Ex_polp_pole_aprox(omega_omegaWG,d,R,phi,z,zp,px,py,pz):     
    ...
    termEx_pole_aprox_re_f = lambda u: np.real(u*rp(u)*(px*term_Ex_px(u) + py*term_Ex_py(u) + 2*1j*pz*u*J1(u)*np.cos(phi))*exp_electron_f(u))
    termEx_pole_aprox_im_f = lambda u: np.imag(u*rp(u)*(px*term_Ex_px(u) + py*term_Ex_py(u) + 2*1j*pz*u*J1(u)*np.cos(phi))*exp_electron_f(u))

    termEx_pole_aprox_int0 = 0
    [inf_limits,sup_limits] = integration_limits(omega_omegaWG,d)
    ...
    for j in range(len(sup_limits)):
        ...
        termEx_pole_aprox_int_re,err = integrate.quad(termEx_pole_aprox_re_f, inf_limits_int, inf_limits_sup) 
        termEx_pole_aprox_int_im,err = integrate.quad(termEx_pole_aprox_im_f, inf_limits_int, inf_limits_sup) 
    
        termEx_pole_aprox_int = termEx_pole_aprox_int_re + 1j*termEx_pole_aprox_int_im
        termEx_pole_aprox_int0 = termEx_pole_aprox_int0 + termEx_pole_aprox_int

    return 1j*0.5*termEx_pole_aprox_int0*(k1**3)  
...
def Ey_polp_num(omega_omegaWG,d,R,phi,z,zp,px,py,pz):     
    ...
    for j in range(len(sup_limits)):
        ...
        termEy_num_int_re,err = integrate.quad(termEx_num_re_f, inf_limits_int, inf_limits_sup) 
        termEy_num_int_im,err = integrate.quad(termEx_num_im_f, inf_limits_int, inf_limits_sup) 
...
def Ey_polp_pole_aprox(omega_omegaWG,d,R,phi,z,zp,px,py,pz):     
    ...
    for j in range(len(sup_limits)):
        ...
        termEy_pole_aprox_int_re,err = integrate.quad(termEx_num_re_f, inf_limits_int, inf_limits_sup) 
        termEy_pole_aprox_int_im,err = integrate.quad(termEx_num_im_f, inf_limits_int, inf_limits_sup) 
...
def Ez_polp_pole_aprox(omega_omegaWG,d,R,phi,z,zp,px,py,pz):     
    ...
    termEz_pole_aprox_re_f = lambda u: np.real(u**2*rp(u)*(J1(u)*term_Ex + 1j*pz*J0(u)*u/alpha_z1(u))*exp_electron_f(u))
    termEz_pole_aprox_im_f = lambda u: np.imag(u**2*rp(u)*(J1(u)*term_Ex + 1j*pz*J0(u)*u/alpha_z1(u))*exp_electron_f(u))
    
    termEz_pole_aprox_int0 = 0
    [inf_limits,sup_limits] = integration_limits(omega_omegaWG,d)
    ...
    for j in range(len(sup_limits)):
        ...
        termEz_pole_aprox_int_re,err = integrate.quad(termEz_pole_aprox_re_f, inf_limits_int, inf_limits_sup) 
        termEz_pole_aprox_int_im,err = integrate.quad(termEz_pole_aprox_im_f, inf_limits_int, inf_limits_sup) 

        termEz_pole_aprox_int = termEz_pole_aprox_int_re + 1j*termEz_pole_aprox_int_im
        termEz_pole_aprox_int0 = termEz_pole_aprox_int0 + termEz_pole_aprox_int

    return termEz_pole_aprox_int0*(k1**3)  
...
def Ez_polp_num(omega_omegaWG,d,R,phi,z,zp,px,py,pz):     
    ...
    termEz_num_re_f = lambda u: np.real(u**2*rp(u)*(J1(u)*term_Ex + 1j*pz*J0(u)*u/alpha_z1(u))*exp_electron_f(u))
    termEz_num_im_f = lambda u: np.imag(u**2*rp(u)*(J1(u)*term_Ex + 1j*pz*J0(u)*u/alpha_z1(u))*exp_electron_f(u))
    ...
    for j in range(len(sup_limits)):
        ...
        termEz_num_int_re,err = integrate.quad(termEz_num_re_f, inf_limits_int, inf_limits_sup) 
        termEz_num_int_im,err = integrate.quad(termEz_num_im_f, inf_limits_int, inf_limits_sup) 

        termEz_num_int = termEz_num_int_re + 1j*termEz_num_int_im
        termEz_num_int0 = termEz_num_int0 + termEz_num_int

    return termEz_num_int0*(k1**3)  
