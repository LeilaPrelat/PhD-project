...
epsilon_pole = 1e-8 ### ayuda a la convergencia. parece que con 1e-3 funciona sin problema
limitt = 70
...
def Ex_PV_pols_pole_aprox(omega_omegaWG,d,R,phi,z,zp,px,py,pz):     
    """    
    Parameters
    ----------
...
#            print(polo_j)
        ...
        termEx_num_re_f = lambda u: np.real(u*(px*term_Ex_px(u) + py*term_Ex_py(u))*exp_electron_f(u)/alpha_z1(u))
        termEx_num_im_f = lambda u: np.imag(u*(px*term_Ex_px(u) + py*term_Ex_py(u))*exp_electron_f(u)/alpha_z1(u))
    
        termEx_num_re_f0 = np.real(qj_polo*(px*term_Ex_px(qj_polo) + py*term_Ex_py(qj_polo))*exp_electron_f(qj_polo)/alpha_z1(qj_polo))
        termEx_num_im_f0 = np.imag(qj_polo*(px*term_Ex_px(qj_polo) + py*term_Ex_py(qj_polo))*exp_electron_f(qj_polo)/alpha_z1(qj_polo))
    
        termEx_num_re_f1 =  lambda u: np.real((termEx_num_re_f(u) - termEx_num_re_f0)*rs(u))
        termEx_num_im_f1 =  lambda u: np.imag((termEx_num_im_f(u) - termEx_num_im_f0)*rs(u))
...
def Ey_PV_pols_pole_aprox(omega_omegaWG,d,R,phi,z,zp,px,py,pz):     
    """    
    Parameters
    ----------
    omegac : omega/c = k0 en 1/micrometros    
...
def Ex_pols_num(omega_omegaWG,d,R,phi,z,zp,px,py,pz):     
    ...
    termEx_num_int_re,err = integrate.quad(termEx_num_re_f, cota_inf_alpha_parallel, cota_sup_alpha_parallel, limit=limitt) 
    termEx_num_int_im,err = integrate.quad(termEx_num_im_f, cota_inf_alpha_parallel, cota_sup_alpha_parallel, limit=limitt) 
...
#        den =  (polo_j*d)**2
#
#        chi_prima = lambda u: d*np.sqrt(epsilon2 - u**2)*k1
#        chi = lambda u:  d*np.sqrt(u**2 - 1)*k1
#        
#        Rj_aux  = lambda u: 2*chi_prima(u)*chi(u)/den
#        
#        term1  = lambda u: (chi(u)**2 + chi_prima(u)**2)/(chi(u)*chi_prima(u))
#        term2  = lambda u:  ((eta*chi(u))**2 + chi_prima(u)**2)/(eta*chi_prima(u))
#        
#        Rj_term_final  = lambda u: 1/(2*term1(u) + term2(u))
#        
#        Rj  = lambda u: Rj_aux(u)*Rj_term_final(u)
#        
#        qj_polo  = polo_j/k1
#        
#        rp = lambda u: Rj(u)*qj_polo/(u - qj_polo - 1j*epsilon_pole) 
#
#
#
#    
...
#
#
#    termEx_pole_aprox_int_re,err = integrate.quad(termEx_num_re_f, cota_inf_alpha_parallel, cota_sup_alpha_parallel, limit=limitt) 
#    termEx_pole_aprox_int_im,err = integrate.quad(termEx_num_im_f, cota_inf_alpha_parallel, cota_sup_alpha_parallel, limit=limitt) 
#
#    termEx_pole_aprox_int = termEx_pole_aprox_int_re + 1j*termEx_pole_aprox_int_im
#
#
...
def Ex_pols_ana_and_PV(omega_omegaWG,d,R,phi,z,zp,px,py,pz):     
    """    
    Parameters
    ----------
    omegac : omega/c = k0 en 1/micrometros    
    ...
    PV_term = Ex_PV_pols_pole_aprox(omega_omegaWG,d,R,phi,z,zp,px,py,pz)
    return 1j*0.5*(np.pi*1j*term0*(k0**3) + PV_term)

#%%
    
def Ey_pols_ana_and_PV(omega_omegaWG,d,R,phi,z,zp,px,py,pz):     
    """    
    Parameters
    ----------
    omegac : omega/c = k0 en 1/micrometros    
    ...
    PV_term = Ey_PV_pols_pole_aprox(omega_omegaWG,d,R,phi,z,zp,px,py,pz)
    return 1j*0.5*(np.pi*1j*term0*(k0**3) + PV_term)
...
def Ey_pols_num(omega_omegaWG,d,R,phi,z,zp,px,py,pz):     
    ...
    termEy_num_int_re,err = integrate.quad(termEx_num_re_f, cota_inf_alpha_parallel, cota_sup_alpha_parallel,  limit=limitt) 
    termEy_num_int_im,err = integrate.quad(termEx_num_im_f, cota_inf_alpha_parallel, cota_sup_alpha_parallel,  limit=limitt) 
...
#  
#
#    termEy_pole_aprox_int_re,err = integrate.quad(termEx_num_re_f, cota_inf_alpha_parallel, cota_sup_alpha_parallel,  limit=limitt) 
#    termEy_pole_aprox_int_im,err = integrate.quad(termEx_num_im_f, cota_inf_alpha_parallel, cota_sup_alpha_parallel,  limit=limitt) 
#
