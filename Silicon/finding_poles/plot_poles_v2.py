...
#    k_medium = k_parallel_medium(omega,epsilon2)   
    ...
    for j in range(len(list_poles)):
        polo_j = list_poles[j]
#            print(polo_j)
        chi_prima = d*np.sqrt(epsilon2*k0**2 - polo_j**2) if (polo_j < k0*np.sqrt(epsilon2)) else 1j*d*np.sqrt(polo_j**2 - epsilon2*k0**2)
        chi = d*np.sqrt(polo_j**2 - k0**2) if (k0 < polo_j) else 1j*d*np.sqrt(k0**2 - polo_j**2)            
        den =  (polo_j*d)**2
...
#    k_medium = k_parallel_medium(omega,epsilon2)   
    ...
    kz1 = np.sqrt(k0**2 - u**2) if (k0 > np.abs(u)) else 1j*np.sqrt(u**2 - k0**2)
    kz2 = np.sqrt(epsilon2*k0**2 - u**2) if (k0*np.sqrt(epsilon2) > np.abs(u)) else 1j*np.sqrt(u**2 - epsilon2*k0**2)
...
k_medium = k_parallel_medium(omega,epsilon2)   
N = 200
list_k_parallel = np.linspace(-k_medium*1.5,k_medium*1.5,N)
...
plt.plot(np.ones(10)*k_air*cte_new, ejey_rp_imag,color = 'grey',label = 'air')
plt.plot(np.ones(10)*k_medium*cte_new, ejey_rp_imag,color = 'black',label = 'medium')
...
plt.plot(np.ones(10)*k_air*cte_new, ejey_rp_real,color = 'grey',label = 'air')
plt.plot(np.ones(10)*k_medium*cte_new, ejey_rp_real,color = 'black',label = 'medium')
plt.legend(loc = 'best',markerscale=mk,fontsize=tamlegend,frameon=True,handletextpad=hp, handlelength=1)
plt.tick_params(labelsize = tamnum,pad = pad)
#plt.yscale('log')
plt.grid(1)
...
plt.tick_params(labelsize = tamnum,pad = pad)
plt.plot(np.ones(10)*k_air*cte_new, ejey_rs_imag,color = 'grey',label = 'air')
plt.plot(np.ones(10)*k_medium*cte_new, ejey_rs_imag,color = 'black',label = 'medium')
plt.legend(loc = 'best',markerscale=mk,fontsize=tamlegend,frameon=True,handletextpad=hp, handlelength=1)
plt.grid(1)
plt.tight_layout()
# os.chdir(path_save)
...
plt.plot(np.ones(10)*k_air*cte_new, ejey_rs_real,color = 'grey',label = 'air')
plt.plot(np.ones(10)*k_medium*cte_new, ejey_rs_real,color = 'black',label = 'medium')
plt.legend(loc = 'best',markerscale=mk,fontsize=tamlegend,frameon=True,handletextpad=hp, handlelength=1)
plt.tick_params(labelsize = tamnum,pad = pad)
plt.grid(1)
plt.tight_layout()
