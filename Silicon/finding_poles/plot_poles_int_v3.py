...
path_save = path_basic + '/poles_int_v3'
path_load = path_ctes
try:
...
#    k_medium = k_parallel_medium(omega,epsilon2)   
    ...
    kz1 = 1j*np.sqrt(u**2 - k0**2)
    kz2 = np.sqrt(epsilon2*k0**2 - u**2) 
...
#    kmedium = k_parallel_medium(omega,epsilon2)
    ...
    for pole in list_poles:
        if pole - deltta > 1:
            ...
            inf_limits.append(1 + 0.0001) ## el integrando diverge en kz = 0 (k parallel = k)
...
def coef_TM_fresnel_int(omega_omegaWG,d):
    ...
    for j in range(len(sup_limits)):
        ...
        inf_limits_int = inf_limits[j]*k0
        inf_limits_sup = sup_limits[j]*k0
        
        termEx_num_int_re,err = integrate.quadrature(rta_real, inf_limits_int, inf_limits_sup) 
        termEx_num_int_im,err = integrate.quadrature(rta_imag, inf_limits_int, inf_limits_sup) 
    
        termEx_num_int = termEx_num_int_re + 1j*termEx_num_int_im
        termEx_num_int0 = termEx_num_int0 + termEx_num_int

    return termEx_num_int0
