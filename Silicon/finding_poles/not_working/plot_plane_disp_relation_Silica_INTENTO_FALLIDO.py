...
def k_parallel_WG_TM1(omega,epsilon2,d,k_parallel):
    ...
    if k_parallel <= epsilon2*k0**2 :
        chi_prima = d*np.sqrt(epsilon2*k0**2 - k_parallel**2)
    else:
        chi_prima = 1j*d*np.sqrt(k_parallel**2 - epsilon2*k0**2)
    
    if k0**2 <= k_parallel**2: 
        chi = d*np.sqrt(k_parallel**2 - k0**2)   
    else:
        chi = 1j*d*np.sqrt(k0**2 - k_parallel**2)   
...
list_omega_omegaWGTM1 = []
list_omega_omegaWGTM2 = []

list_omega_omegaWGTE1 = []
list_omega_omegaWGTE2 = []
...
#    list_x.append(omega/omegaWG)
    ...
    init_cond = k_air
    if kTM1 == 0:
        init_condTM1 = init_cond
    
    else:
        init_condTM1 = list_y_WG_TM1[kTM1 - 1]
    
    if kTM2 == 0:
        init_condTM2 = 0
    else:
        init_condTM2 = list_y_WG_TM2[kTM2 - 1]
        
    
    if kTE1 == 0:
        init_condTE1 = 0
    else:
        init_condTE1 = list_y_WG_TE1[kTE1 -1]
        
    if kTE2 == 0:
        init_condTE2 = 0
    else:    
        init_condTE2 = list_y_WG_TE2[kTE2 -1]       


   
    resTM1 = minimize(k_parallel_WG_TM_minimiza1, init_condTM1, method='Nelder-Mead', tol=1e-9, 
                   options={'maxiter':1500})
    
#    res1B = minimize(k_parallel_WG_TM_minimiza1, k_medium, method='Nelder-Mead', tol=1e-9, 
#                   options={'maxiter':1500})

    resTM2 = minimize(k_parallel_WG_TM_minimiza2, init_condTM2, method='Nelder-Mead', tol=1e-9, 
                   options={'maxiter':1500})
    
    
    resTE1 = minimize(k_parallel_WG_TE_minimiza1, init_condTE1, method='Nelder-Mead', tol=1e-9, 
                   options={'maxiter':1500})
...
#                   options={'maxiter':1500})
#    
    resTE2 = minimize(k_parallel_WG_TE_minimiza2, init_condTE2, method='Nelder-Mead', tol=1e-9, 
                   options={'maxiter':1500})
    
    
    
    if kz1 < resTM1.x[0] < kz2:

        list_y_WG_TM1.append(resTM1.x[0]*cte_new)
        list_omega_omegaWGTM1.append(omega_omegaWG)
        kTM1 = kTM1 + 1
        
    if kz1 < resTM2.x[0] < kz2:
        list_y_WG_TM2.append(resTM2.x[0]*cte_new)
        list_omega_omegaWGTM2.append(omega_omegaWG)
        kTM2 = kTM2 + 1

    if kz1 < resTE1.x[0] < kz2:
        list_y_WG_TE1.append(resTE1.x[0]*cte_new)
        list_omega_omegaWGTE1.append(omega_omegaWG)
        kTE1 = kTE1 + 1
    
    if kz1 < resTE2.x[0] < kz2:
        list_y_WG_TE2.append(resTE2.x[0]*cte_new)
        list_omega_omegaWGTE2.append(omega_omegaWG)
        kTE2 = kTE2 + 1   
#    if kz1 < res1B.x[0] < kz2:
#    
#        list_y_WG_TM1B.append(res1B.x[0]*cte_new)
#    
#    if kz1 < res2B.x[0] < kz2:
...
plt.plot(list_y_WG_TE1,list_omega_omegaWGTE1,'.-',color = 'red',ms = 3, label = 'WG TE')
#plt.plot(list_y_WG_TE1B,list_omega_omegaWG,'.-',color = 'red',ms = 3)

plt.plot(list_y_WG_TM1,list_omega_omegaWGTM1,'.-',color = 'blue',ms = 3, label = 'WG TM')
#plt.plot(list_y_WG_TM1B,list_omega_omegaWG,'.-',color = 'blue',ms = 3)

plt.plot(list_y_WG_TE2,list_omega_omegaWGTE2,'.-',color = 'red',ms = 3)
#plt.plot(list_y_WG_TE2B,list_omega_omegaWG,'.-',color = 'red',ms = 3)
#
#plt.plot(list_y_WG_TM2A,list_omega_omegaWG,'.-',color = 'blue',ms = 3)
plt.plot(list_y_WG_TM2,list_omega_omegaWGTM2,'.-',color = 'blue',ms = 3)
