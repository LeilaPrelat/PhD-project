...
try:
    ...
    from green_self_image import green_self_ana2,green_self_num,green_self_pole_aprox
except ModuleNotFoundError:
    print('green_self_image.py no se encuentra en ' + path_constants)
...
def EELS_film_ana_f(omegac,epsi1,epsi3,d_nano,int_v,b,zp,omega0,kappa_factor_omega0,kappa_r_factor,dir_dip_moment):     
    """    
    Parameters
    ----------
...
#    k1_2 = (k0*cte1)**2
 #   n_v1 = int_v/cte1
#    k1_2 = k1**2
    px_v,py_v,pz_v = dipole_moment_pole_aprox(omegac,epsi1,epsi3,d_nano,int_v,b,zp,omega0,kappa_factor_omega0,kappa_r_factor)
...
#    px_tot_2 = np.abs(px_v)**2 + np.abs(py_v)**2 + np.abs(pz_v)**2 
    ...
    if dir_dip_moment == 'all':
        Green_self = rtaself_x*(np.abs(px_v)**2) + rtaself_y*(np.abs(py_v)**2)  + rtaself_z*(np.abs(pz_v)**2)
    elif dir_dip_moment == 'x':
        Green_self = rtaself_x*(np.abs(px_v)**2) 
        
    elif dir_dip_moment == 'z':
        Green_self = rtaself_z*(np.abs(pz_v)**2) 
...
def EELS_film_num_f(omegac,epsi1,epsi3,d_nano,int_v,b,zp,omega0,kappa_factor_omega0,kappa_r_factor,dir_dip_moment):     
    """    
    Parameters
    ----------
    omegac : omega/c = k0 en 1/micrometros    
...
#    k1_2 = (k0*cte1)**2
 #   n_v1 = int_v/cte1
#    k1_2 = k1**2
    px_v,py_v,pz_v = dipole_moment_num(omegac,epsi1,epsi3,d_nano,int_v,b,zp,omega0,kappa_factor_omega0,kappa_r_factor)
#    print(px_v,py_v,pz_v)
#    px_tot_2 = np.abs(px_v)**2 + np.abs(py_v)**2 + np.abs(pz_v)**2 
...
def EELS_film_pole_aprox_f(omegac,epsi1,epsi3,d_nano,int_v,b,zp,omega0,kappa_factor_omega0,kappa_r_factor,dir_dip_moment):     
    """    
    Parameters
    ----------
    omegac : omega/c = k0 en 1/micrometros    
