...
try:
    ...
    from potential import potential_ana_resonance_v1,potential_ana_resonance_v2, potential_num_resonance, potential_pole_aprox_resonance_v1,potential_pole_aprox_resonance_v2
...
title4 = r'$\hbar\omega$ = %i meV, z = %i nm, $\phi$ = %i' %(energy0*1e3,z*1e3,phi)

labelp = r'_res_d%inm_E0%imeV' %(d_nano,energy0*1e3)
...
def function_num(R_nano):
    omegac0 = energy0/aux 
    R = R_nano*1e-3

    potential = potential_num_resonance(omegac0,epsilon_Silica,d_nano,int_v,b,zp,R,phi,z)
    # print(rta)
    return potential
...
def function_ana_v1(R_nano):
    omegac0 = energy0/aux 
    R = R_nano*1e-3

    potential  = potential_ana_resonance_v1(omegac0,epsilon_Silica,d_nano,int_v,b,zp,R,phi,z)
...
def function_ana_v2(R_nano):
    omegac0 = energy0/aux 
    R = R_nano*1e-3

    potential  = potential_ana_resonance_v2(omegac0,epsilon_Silica,d_nano,int_v,b,zp,R,phi,z)
    
    return potential

#%%

def function_pole_approx_v1(R_nano):
    omegac0 = energy0/aux 
    R = R_nano*1e-3

    potential  = potential_pole_aprox_resonance_v1(omegac0,epsilon_Silica,d_nano,int_v,b,zp,R,phi,z)
    
    return potential


def function_pole_approx_v2(R_nano):
    omegac0 = energy0/aux 
    R = R_nano*1e-3

    potential  = potential_pole_aprox_resonance_v2(omegac0,epsilon_Silica,d_nano,int_v,b,zp,R,phi,z)
...
for value in listx: 
    ...
    listy_re_pole_v1.append(np.real(y_pole_v1))
    listy_re_pole_v2.append(np.real(y_pole_v2))
...
#    listy_im_num.append(np.imag(y_num))
    ...
    listy_im_pole_v1.append(np.imag(y_pole_v1))
    listy_im_pole_v2.append(np.imag(y_pole_v2))
...
plt.plot(listx,listy_re_pole_v1,'.-',ms = 3,color = 'darkred',label = 'PP num 1' )
plt.plot(listx,listy_re_pole_v2,'--',ms = 3,color = 'darkred',label = 'PP num 2'  )
...
#plt.plot(listx,listy_re_num,'.',ms = ms,color = 'lightseagreen',label = 'full numerical')
plt.plot(listx,listy_im_pole_v1,'.-',ms = 3,color = 'darkred',label = 'PP num 1' )
plt.plot(listx,listy_im_pole_v2,'--',ms = 3,color = 'darkred',label = 'PP num 2'  )
