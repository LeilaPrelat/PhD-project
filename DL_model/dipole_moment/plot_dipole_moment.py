...
try:
    ...
    from dipole_moment import Efield_tot, Efield_tot_QE 
...
int_v = 600
v = c/int_v
b = -0.02 #electron position in z < 0 (arriba del plano)
#omega = 0.7*1e12
cota = 2
epsi1,epsi2 = 12,2.4
hbgama = 0.01
...
if plot_xy_3D ==1 :
    ...
    title2A = r'$\epsilon_2$ = %.1f, $E_{bulk}$ = %.2feV, $\hbar\gamma$ = %.4feV, $z_p=%.2f\mu$m, z = %i$\mu$m' %(epsi2,Ebulk,hbgama,zp,z0) 
    title = title1 + '\n'  + title2A   + '\n' + title3 + ', ' + title4
    ...
    list_OmegaTHz = np.linspace(0.001,2.01,51)
    list_E = np.array(list_OmegaTHz)*1e12*hb
    
 #   list_OmegaTHz = np.linspace(1,1e3,101)
    ...
    title2B = r'$\omega$ = %.2fTHz, $\epsilon_2$ = %.1f, $E_{bulk}$ = %.2feV, $\hbar\gamma$ = %.4feV' %(omegaTHz0,epsi2,Ebulk,hbgama) 
...
if plot_xy_3D ==1 : 
    ...
    def functionQE_real(x,y):
        rta = Efield_tot_QE(omegac0,epsi1,epsi2,omega_bulk,hbgama,d,epsilon_b,int_v,b,zp,x,y,z0,omega0,kappa_factor_omega0,kappa_r_factor)
        return rta.real
    
    def function_real(x,y):
        rta = Efield_tot(omegac0,epsi1,epsi2,omega_bulk,hbgama,d,epsilon_b,int_v,b,zp,x,y,z0,omega0,kappa_factor_omega0,kappa_r_factor)
        return rta.real
...
#%%
    ...
    for omegaTHz in list_OmegaTHz:
        ...
        rta = Efield_tot_QE(omegac,epsi1,epsi2,omega_bulk,hbgama,d,epsilon_b,int_v,b,zp,x0,y0,z0,omega0,kappa_factor_omega0,kappa_r_factor)
        listyQE_re.append(rta.real)
        listyQE_im.append(rta.imag)
        
        if plot_sin_QE == 1:
            rta = Efield_tot(omegac,epsi1,epsi2,omega_bulk,hbgama,d,epsilon_b,int_v,b,zp,x0,y0,z0,omega0,kappa_factor_omega0,kappa_r_factor)
...
#%%
    ...
    for z in list_z:
        ...
        z = np.round(z,8)
        rta = Efield_tot_QE(omegac0,epsi1,epsi2,omega_bulk,hbgama,d,epsilon_b,int_v,b,zp,x0,y0,z,omega0,kappa_factor_omega0,kappa_r_factor)
        listyQE_re.append(rta.real)
        listyQE_im.append(rta.imag)
        
        if plot_sin_QE == 1:
            rta = Efield_tot(omegac0,epsi1,epsi2,omega_bulk,hbgama,d,epsilon_b,int_v,b,zp,x0,y0,z,omega0,kappa_factor_omega0,kappa_r_factor)
            listy_re.append(rta.real)
            listy_im.append(rta.imag)
