...
#    phi = 0  # green tensor evaluated in 
    ...
    cota_sup2 = 60
############ I0 5 ########################################################
    Int05_B_function_re = lambda u,w: J0(w,u)*expB(u)*cos_electron(w)*(u**2 - 1)
    Int05_B_function_im = lambda u,w: J0(w,u)*expB(u)*sin_electron(w)*(u**2 - 1)

    int05B_re,err = integrate.dblquad(Int05_B_function_re, -cota_sup2, cota_sup2, 0, cota_sup1)
    int05B_im,err = integrate.dblquad(Int05_B_function_im, -cota_sup2, cota_sup2, 0, cota_sup1)
#########################################################################
...
#    phi = 0  # green tensor evaluated in 
 ...
 #   exp_electron = lambda w: np.exp(1j*omegac*w*int_v)
    ...
    cota_sup2A = 60
############ parte A ########################################################
    Int05_A_function_re = lambda u,w: J0(w,u)*u*(cosA(u)*cos_electron(w) - sinA(u)*sin_electron(w))*(alpha_z1(u) + 1/alpha_z1(u))
...
###################### parte B ###############################################
    ...
    Int05_B_function_re = lambda u,w: -J0(w,u)*u*expB(u)*sin_electron(w)*(alpha_z2(u) - 1/alpha_z2(u)) #hay un 1j afuera, por eso la parte real es con sin y con un menos

    Int05_B_function_im = lambda u,w: J0(w,u)*u*expB(u)*cos_electron(w)*(alpha_z2(u) - 1/alpha_z2(u))
                                                                    # saque el 1j afuera asi que hay
                                                                    # un menos en 1/alfa2
                                                                                                            
    int05B_re,err = integrate.dblquad(Int05_B_function_re, -cota_sup2A, cota_sup2A, cota_inf1, cota_sup1A)
    int05B_im,err = integrate.dblquad(Int05_B_function_im, -cota_sup2A, cota_sup2A, cota_inf1, cota_sup1A)
