...
err = 'green_tensor_ref_xx_Ag.py no se encuentra en ' + path_basic
try:
    sys.path.insert(1, path_basic)
    from green_tensor_ref_Ag import green_tensor_NUM_QE, green_tensor_NUM
except ModuleNotFoundError:
    print(err)
path_save = path_basic + '/' + 'green_tensor_ref_xx_Ag'
...
epsilon_b = 4        
energy_bulk = 9.17
omega_bulk, hbar_gamma_in, d = energy_bulk/hb, 0.0001, 0.0001 

title1 = r'$\epsilon_1$ = %i, $\epsilon_2$ = %i, $\epsilon_b$ = %i, x = %i$\mu$m, y = %i$\mu$m, zp = %i$\mu$m' %(epsi1,epsi2,epsilon_b,x,y,zp)
title2 = r'x = %i, y = %i, yD = %i, zD = %i' %(x,y,yD,zD) 
title3 = r'$E_{bulk}$ = %.2feV, $\hbar \gamma$ = %.4feV, d = %.4f$\mu$m' %(energy_bulk,hbar_gamma_in,d)

def functionQE(omegac,z,xD):
    return green_tensor_NUM_QE(omegac,epsi1,epsi2,omega_bulk,hbar_gamma_in,d,epsilon_b,x,y,z,xD,yD,zD,zp)

def function(omegac,z,xD):    
    return green_tensor_NUM(omegac,epsi1,epsi2,omega_bulk,hbar_gamma_in,d,epsilon_b,x,y,z,xD,yD,zD,zp)    
...
if plot_vs_z == 1:
    ...
    labelx = '$z$ [$\mu$m]'
    label1 = '_vs_z'
    title2 = title2 +  r', $\omega$ = %.2f THz, $x_D$ = %i$\mu$m' %(omegaTHz0,xD0)
...
if plot_vs_omegaTHz == 1:
    ...
    list_OmegaTHz = np.linspace(1*1e3,2*1e3,101)
    list_x = list_OmegaTHz 
    labelx = '$\omega$ [THz]'
    label1 = '_vs_OmegaTHz'
    title2 = title2 + r', z = %.2f $\mu$m, $x_D$ = %i$\mu$m' %(z0,xD0)
...
if plot_vs_xD == 1:
    ...
    labelx = r'$x_D$ [$\mu$m]'
    label1 = '_vs_xD'
    title2 = title2 +  r', $\omega$ = %.2f THz, z = %.2f$\mu$m' %(omegaTHz0,z0)
...
# elif graphs_vs_omegaTHz == 1 and x==0 and y==0:
#     labely = r'Re(Reflected Green Tensor)'
#     fig,axs = plt.subplots(2,1, sharex=True, facecolor='w', figsize = tamfig)
#     plt.subplots_adjust(hspace =hspace,wspace = wspace)
    
#     axs[0].plot(list_x,total1_re,'.',ms = ms,color = 'black',label = 'numerical QE')
#     axs[1].plot(list_x,total2_re,'-',color = 'green',label =  'numerical')
