...
def Efield_NUM_QE(omegac,epsi1,epsi2,omega_bulk,gamma_in,d,epsilon_b,int_v,b,zp,x,y,z):
    """
    Parameters
    ----------
...
def Efield_NUM(omegac,epsi1,epsi2,omega_bulk,gamma_in,d,epsilon_b,int_v,b,zp,x,y,z):
    """
    Parameters
    ----------
    omegac : omega/c = k0 en 1/micrometros    
...
def Efield_NUM_SP(omegac,epsi1,epsi2,omega_bulk,gamma_in,d,epsilon_b,int_v,b,zp,z):
    """
    Parameters
    ----------
    omegac : omega/c = k0 en 1/micrometros    
...
#    E = omegac*aux  
    ...
    z_dip_barra = k1*(z + np.abs(b) + 2*zp)      
...
####################################################################
    ...
    int_re,err = integrate.dblquad(Int_function_re, 1, cota_sup1, -cota_sup2, cota_sup2)
    int_im,err = integrate.dblquad(Int_function_im, 1, cota_sup1, -cota_sup2, cota_sup2)
...
def Efield_ana1(omegac,epsi1,epsi2,omega_bulk,gamma_in,d,epsilon_b,int_v,b,x,y,z):
    """
    Parameters
    ----------
    omegac : omega/c = k0 en 1/micrometros    
...
#    E = omegac*aux  
    ...
    exp_electron = np.exp(-alfa_p*k1*np.abs(z+b))
...
def Efield_ana1(omegac,epsi1,epsi2,omega_bulk,gamma_in,d,epsilon_b,int_v,b,zp,z):
    """
    Parameters
    ----------
    omegac : omega/c = k0 en 1/micrometros    
...
#    k0 = omegac
    ...
    exp_electron = np.exp(-alfa_p*k1*(z + np.abs(b) + 2*zp))
...
def Efield_ana1b(omegac,epsi1,epsi2,omega_bulk,gamma_in,d,epsilon_b,int_v,b,zp,z):
    """
    Parameters
    ----------
    omegac : omega/c = k0 en 1/micrometros    
...
#    k0 = omegac
    ...
    cte = np.pi*k1_2*Rp*(alfa_p**(5/2))/4
    cte2 = 1/np.sqrt(alfa_p + n_v1) + 1/np.sqrt(alfa_p - n_v1) 
    cte3 = np.sqrt(-1j)*(1 + 1j) + np.sqrt(1j)*(1-1j)  
...
def Efield_ana2(omegac,epsi1,epsi2,omega_bulk,gamma_in,d,epsilon_b,int_v,b,zp,z):
    """
    Parameters
    ----------
    omegac : omega/c = k0 en 1/micrometros    
...
#    k0 = omegac
    ...
    K0 = special.kn(0, int_v*k1*(z + np.abs(b) + 2*zp)/n1) 
    
    cte = k1_2*Rp*alfa_p*(k1*(z + np.abs(b) + 2*zp) + alfa_p)
