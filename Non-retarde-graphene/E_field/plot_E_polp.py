...
path_save = path_basic + '/' + 'E_polp_v5'
#if not os.path.exists(path_save):
#    print('Creating folder to save graphs')
#    os.mkdir(path_save)

err = 'E_polp_v2.py no se encuentra en ' + path_basic
try:
    sys.path.insert(1, path_basic)
    from E_polp_v5 import Ex_polp_num, Ex_polp_pole_aprox, Ex_polp_ana, Ey_polp_num, Ey_polp_pole_aprox, Ey_polp_ana, Ez_polp_num, Ez_polp_pole_aprox, Ez_polp_ana
...
title1 = r'z = %inm, $z_p$=%inm' %(z0*1e3,zp*1e3)
labelp = r'_px%i_py%i_pz%i_phi%i' %(px,py,pz,cte_phi0)
title = title1 + '\n' + title0
...
if plot_vs_R == 1: 
    E0 = 43 # meV 
    labelx = 'R [nm]'  
    title = title + ', ' + 'E = %.2f meV' %(E0)
    label1 = 'vs_R' + labelp + '_E%i' %(E0)
    ...
    label1 = 'vs_E' + labelp
    listx =np.linspace(15,100,N)

#omegaWG = (np.pi/d)/(np.sqrt(epsilon2-1)) # el c esta metido en aux

#%%

def Ex_real_ana(R_nano,energy0):
    R = R_nano*1e-3 
    omegac0 = energy0*1e-3/aux 
    rta = Ex_polp_ana(omegac0,hbmu,hbgama,R,phi0,z0,zp,px,py,pz)
    
    return rta.real

def Ex_imag_ana(R_nano,energy0):
    R = R_nano*1e-3 
    omegac0 = energy0*1e-3/aux     
    rta = Ex_polp_ana(omegac0,hbmu,hbgama,R,phi0,z0,zp,px,py,pz)
    return rta.imag

def Ey_real_ana(R_nano,energy0):
    R = R_nano*1e-3 
    omegac0 = energy0*1e-3/aux 
    rta = Ey_polp_ana(omegac0,hbmu,hbgama,R,phi0,z0,zp,px,py,pz)
    
    return rta.real

def Ey_imag_ana(R_nano,energy0):
    R = R_nano*1e-3 
    omegac0 = energy0*1e-3/aux     
    rta = Ey_polp_ana(omegac0,hbmu,hbgama,R,phi0,z0,zp,px,py,pz)
    return rta.imag

def Ez_real_ana(R_nano,energy0):
    R = R_nano*1e-3 
    omegac0 = energy0*1e-3/aux 
    rta = Ez_polp_ana(omegac0,hbmu,hbgama,R,phi0,z0,zp,px,py,pz)
    
    return rta.real

def Ez_imag_ana(R_nano,energy0):
    R = R_nano*1e-3 
    omegac0 = energy0*1e-3/aux     
    rta = Ez_polp_ana(omegac0,hbmu,hbgama,R,phi0,z0,zp,px,py,pz)
    return rta.imag
...
def Ex_real_num(R_nano,energy0):
#    omega0 = energy0*1e-3/aux 
#    omega_omegaWG = omega0/omegaWG
    R = R_nano*1e-3
    omegac0 = energy0*1e-3/aux 
    rta = Ex_polp_num(omegac0,hbmu,hbgama,R,phi0,z0,zp,px,py,pz)
    
    return rta.real

def Ex_imag_num(R_nano,energy0):
#    omega0 = energy0*1e-3/aux    
#    omega_omegaWG = omega0/omegaWG
    R = R_nano*1e-3
    omegac0 = energy0*1e-3/aux     
...
def Ey_real_num(R_nano,energy0):
#    omega0 = energy0*1e-3/aux    
#    omega_omegaWG = omega0/omegaWG
    R = R_nano*1e-3
    omegac0 = energy0*1e-3/aux 
    rta = Ey_polp_num(omegac0,hbmu,hbgama,R,phi0,z0,zp,px,py,pz)
    
    return rta.real

def Ey_imag_num(R_nano,energy0):
#    omega0 = energy0*1e-3/aux    
#    omega_omegaWG = omega0/omegaWG
    R = R_nano*1e-3
    omegac0 = energy0*1e-3/aux     
...
def Ez_real_num(R_nano,energy0):
#    omega0 = energy0*1e-3/aux    
#    omega_omegaWG = omega0/omegaWG
    R = R_nano*1e-3
    omegac0 = energy0*1e-3/aux 
    rta = Ez_polp_num(omegac0,hbmu,hbgama,R,phi0,z0,zp,px,py,pz)
    
    return rta.real

def Ez_imag_num(R_nano,energy0):
#    omega0 = energy0*1e-3/aux    
#    omega_omegaWG = omega0/omegaWG
    R = R_nano*1e-3
    omegac0 = energy0*1e-3/aux    
...
def Ex_real_pole_aprox(R_nano,energy0):
#    omega0 = energy0*1e-3/aux 
#    omega_omegaWG = omega0/omegaWG
    R = R_nano*1e-3
    omegac0 = energy0*1e-3/aux  
    rta = Ex_polp_pole_aprox(omegac0,hbmu,hbgama,R,phi0,z0,zp,px,py,pz)
    
    return rta.real

def Ex_imag_pole_aprox(R_nano,energy0):
#    omega0 = energy0*1e-3/aux 
#    omega_omegaWG = omega0/omegaWG
    R = R_nano*1e-3
    omegac0 = energy0*1e-3/aux      
...
def Ey_real_pole_aprox(R_nano,energy0):
#    omega0 = energy0*1e-3/aux 
#    omega_omegaWG = omega0/omegaWG
    R = R_nano*1e-3
    omegac0 = energy0*1e-3/aux  
    rta = Ey_polp_pole_aprox(omegac0,hbmu,hbgama,R,phi0,z0,zp,px,py,pz)
    
    return rta.real

def Ey_imag_pole_aprox(R_nano,energy0):
#    omega0 = energy0*1e-3/aux 
#    omega_omegaWG = omega0/omegaWG
    R = R_nano*1e-3
    omegac0 = energy0*1e-3/aux     
...
def Ez_real_pole_aprox(R_nano,energy0):
#    omega0 = energy0*1e-3/aux 
#    omega_omegaWG = omega0/omegaWG
    R = R_nano*1e-3
    omegac0 = energy0*1e-3/aux  
    rta = Ez_polp_pole_aprox(omegac0,hbmu,hbgama,R,phi0,z0,zp,px,py,pz)
    
    return rta.real

def Ez_imag_pole_aprox(R_nano,energy0):
#    omega0 = energy0*1e-3/aux 
#    omega_omegaWG = omega0/omegaWG
    R = R_nano*1e-3
    omegac0 = energy0*1e-3/aux      
...
if plot_vs_R == 1: 
    ...
    listy_re_pole_aproxG3 = []
    listy_im_pole_aproxG3 = []
      
#    j = 0 
    for value in listx: 

        y_re_anaG1 = Ex_real_ana(value,E0)
        y_im_anaG1 = Ex_imag_ana(value,E0)        


        y_re_numG1 = Ex_real_num(value,E0)
        y_im_numG1 = Ex_imag_num(value,E0)  
        
    
        y_re_pole_aproxG1 = Ex_real_pole_aprox(value,E0)
        y_im_pole_aproxG1 = Ex_imag_pole_aprox(value,E0)  
        ...
        y_re_anaG2 = Ey_real_ana(value,E0)
        y_im_anaG2 = Ey_imag_ana(value,E0)        


        y_re_numG2 = Ey_real_num(value,E0)
        y_im_numG2 = Ey_imag_num(value,E0)  
        
         
        y_re_pole_aproxG2 = Ey_real_pole_aprox(value,E0)
        y_im_pole_aproxG2 = Ey_imag_pole_aprox(value,E0)  
        ...
        y_re_anaG3 = Ez_real_ana(value,E0)
        y_im_anaG3 = Ez_imag_ana(value,E0)        


        y_re_numG3 = Ez_real_num(value,E0)
        y_im_numG3 = Ez_imag_num(value,E0)  
        
       
        y_re_pole_aproxG3 = Ez_real_pole_aprox(value,E0)
        y_im_pole_aproxG3 = Ez_imag_pole_aprox(value,E0)  
        
        
        listy_re_pole_aproxG3.append(y_re_pole_aproxG3)
        listy_im_pole_aproxG3.append(y_im_pole_aproxG3)           
...
if plot_vs_E == 1: 
    ...
    for value in listx: 
        ...
        y_re_anaG3 = Ez_real_ana(R0,value)
        y_im_anaG3 = Ez_imag_ana(R0,value)        


        y_re_numG3 = Ez_real_num(R0,value)
        y_im_numG3 = Ez_imag_num(R0,value)  
        
       
        y_re_pole_aproxG3 = Ez_real_pole_aprox(R0,value)
        y_im_pole_aproxG3 = Ez_imag_pole_aprox(R0,value)  
...
graph(title,labelx,'Re($E_z$)',tamfig,tamtitle,tamletra,tamnum,labelpadx,labelpady,pad)
plt.plot(listx,listy_re_anaG3,'.-',ms = ms,color = 'purple',label = 'PP analytical')
#plt.plot(listx,listy_re_pole_aproxG3,'.',ms = ms,color = 'darkred',label = 'PP numerical')
plt.plot(listx,listy_re_numG3,'.-',ms = ms,color = 'lightseagreen',label = 'full numerical')
plt.legend(loc = 'best',markerscale=mk,fontsize=tamlegend,frameon=0,handletextpad=0.2, handlelength=length_marker)
plt.tight_layout()
plt.savefig( 'Re_Ez' + label1 + '.png', format='png')   

graph(title,labelx,'Im($E_z$)',tamfig,tamtitle,tamletra,tamnum,labelpadx,labelpady,pad)
plt.plot(listx,listy_im_anaG3,'.-',ms = ms,color = 'purple',label = 'PP analytical')
#plt.plot(listx,listy_im_pole_aproxG3,'.',ms = ms,color = 'darkred',label = 'PP numerical')
plt.plot(listx,listy_im_numG3,'.-',ms = ms,color = 'lightseagreen',label = 'full numerical')
plt.legend(loc = 'best',markerscale=mk,fontsize=tamlegend,frameon=0,handletextpad=0.2, handlelength=length_marker)
plt.tight_layout()
plt.savefig( 'Im_Ez' + label1 + '.png', format='png')   
