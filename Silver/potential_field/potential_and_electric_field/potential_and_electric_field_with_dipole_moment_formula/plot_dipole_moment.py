...
title1 = r'v = c/%i, $\hbar\omega_0$ = %.2f eV, $z_p$ = %i nm, $\epsilon_b$ = %i' %(int_v, energy0_pol,zp*1e3,epsilon_b)     
#title2 = r'$\hbar\mu$ = %.2feV, $\hbar\gamma$ = %.4feV' %(hbmu,hbgama) 
#title3 = r'$z_p$=%inm, px=%i, py=%i, pz=%i' %(zp*1e3,px,py,pz)
title4 = r'b = %i nm, d = %i nm, $\hbar\omega$ = %.2f eV, $\hbar\gamma_{in}$ = %i meV' %(b*1e3,d_nano,hbomega_DL,hbgamma_DL*1e3)
labelp = r'_E0_%imeV_d%inm' %(energy0_pol,d_nano)
...
#
#ejey_aux = np.linspace(np.min([np.min(listy_re_anav2),np.min(listy_re_num)]), np.max([np.max(listy_re_anav2),np.max(listy_re_num)]) , 10)
#for x in [x1,x2,x3]:
