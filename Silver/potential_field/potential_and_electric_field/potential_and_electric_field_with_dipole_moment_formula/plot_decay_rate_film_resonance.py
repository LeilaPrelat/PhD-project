...
try:
    ...
    from decay_rate_film_resonance import EELS_film_ana_f, EELS_film_num_f, EELS_film_pole_aprox_f, EELS_dir_ana_f
...
def function_imag_num(energy0,int_v,zp_nano):
    ...
    rta = EELS_film_num_f(omegac0,epsi1,epsi3,d_nano,int_v,b,zp)
...
def function_imag_ana_tot(energy0,int_v,zp_nano):
    omegac0 = energy0/aux 
    zp = zp_nano*1e-3
...
if plot_vs_zp == 1 : 
    ...
    if d_nano == 100:
        ...
        int_v0 = 5 ## deberia ser 150 (disp relation) pero funciona con 10 <--- problema con la relacion de dispersion
        E0 = 2 #eV
 

       
        
        listx = np.linspace(0.01,250,N)
        
        
    elif d_nano == 1:
        int_v0 = 10
        E0 = 1
        listx = np.linspace(1,250,N)
        
#    elif d_nano == 5:
#        int_v0 = 5
#        E0 = 2.75
#        listx = np.linspace(0.01,2000,N)
    ...
    lambda_p_v = Silver_lambda_p(E0,epsi1,epsi3)*d_micros
    kp = np.real(2*np.pi/lambda_p_v)
    
    labelx = r'$z_p$ [nm]'  
    if d_nano != 100:
        title4 = title4 + ', ' + r'v = c/%i, $\hbar\omega$ = %.2f eV, $k_p$ = %.2f 1/$\mu$m' %(int_v0,E0,kp)
    else:
        title4 = title4 + ', ' + r'v = c/%.2f, $\hbar\omega$ = %.2f eV, $k_p$ = %.2f 1/$\mu$m' %(int_v0,E0,kp)
...
if plot_vs_E == 1: 
    ...
    for value in listx: 
        ...
        y_im_ana_tot = function_imag_ana_tot(value,int_v0,zp0)        
        listy_im_ana_tot.append(y_im_ana_tot)
...
#        print(value2)
        ...
        y_im_ana_tot = function_imag_ana_tot(E0,value2,zp0)        
        listy_im_ana_tot.append(y_im_ana_tot)
    ...
    for value in listx: 
        ...
        y_im_ana_tot = function_imag_ana_tot(E0,int_v0,value)        
        listy_im_ana_tot.append(y_im_ana_tot)
...
#%%
peaks, _ = find_peaks(listy_im_num, height=0)
maxi = listx[peaks]
listy_aux  = np.linspace(np.min(listy_im_num), np.max(listy_im_num), 10)
print(E0,maxi)
...
graph(title,labelx,r'$\Gamma_{\rm SP}/\Gamma$',tamfig,tamtitle,tamletra,tamnum,labelpadx,labelpady,pad)
plt.plot(listx,listy_im_ana_tot,'.-',ms = ms,color = 'purple')#plt.plot(np.ones(10)*maxi, listy_aux,'-k')
plt.legend(loc = 'best',markerscale=1.5,fontsize=tamlegend,frameon=0.1,handletextpad=0.2, handlelength=length_marker)
plt.tight_layout()
#if plot_vs_c == 1:
#    plt.yscale('log')
