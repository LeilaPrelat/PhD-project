...
path_save = path_basic + '/' + 'decay_rate_film_div_tot_ana'
...
try:
    ...
    from decay_rate_film_resonance_div_tot_ana import EELS_film_ana_f, EELS_dir_ana_f
...
def function_imag_ana_epsilon(energy0,int_v,zp_nano):
    ...
    rta = EELS_dir_ana_f(omegac0,epsi1,epsi3,d_nano,int_v,b,zp)
...
if plot_color_maps == 1:
    ...
    def function_imag_ana_par_3D(energy0,zp_nano):
        ...
        rta_par, rta_perp = EELS_film_ana_f(omegac0,epsi1,epsi3,d_nano,int_v0,b,zp)
...
if plot_vs_c == 1 :
    ...
    labelx = r'v/c'   
    title5 = r'$\hbar\omega$ = %i eV, $z_p$ = %i nm' %(E0,zp0)
    label1 = 'vs_v' + labelp
...
if plot_vs_E ==1 :
    ...
    title5 = r'v = c/%i, $z_p$ = %i nm' %(int_v0,zp0)
    label1 = 'vs_E' + labelp
#    listx = np.linspace(0.0001,2,N)
...
if plot_vs_zp == 1 : 
    ...
    title5 = r'v = c/%i, $\hbar\omega$ = %.2f eV' %(int_v0,E0)
...
#    del sns
    ...
    im = plt.imshow(Z_pole_approx_par, extent = limits, cmap=plt.cm.hot, aspect='auto', interpolation = 'none',origin = 'lower') 

    cbar = plt.colorbar(im, extend='both', fraction=0.046, pad=0.04)
    cbar.ax.tick_params(labelsize = tamnum)
    if paper == 0:
        cbar.set_label('log($\Gamma_{SP}$)',fontsize=tamlegend,labelpad = 1)
    plt.tight_layout()
    os.chdir(path_save)
