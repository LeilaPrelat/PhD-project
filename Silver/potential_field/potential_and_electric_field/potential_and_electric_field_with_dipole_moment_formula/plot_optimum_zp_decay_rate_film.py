...
title2 = r'b = %i nm, v = c/%i, d = %i nm, Silver' %(b*1e3,int_v, d_nano)
#title2 = r'$\hbar\gamma_{in}$ = %i meV, $\epsilon_b$ = %i' %(hbgamma_DL*1e3,epsilon_b)
labelp = r'_d%inm_v%i_E0%imeV' %(d_nano,int_v,energy0_pol*1e3)
title = title1 + '\n' + title2


N = 100
list_freq = np.linspace(0.5,4,500)
list_zp = np.linspace(0.01,600,1000)
...
#    plt.plot(list_freq,list_lambda_p,'.-',ms = ms,color = 'lightseagreen',label = r'$\lambda_p$')
    ...
    plt.tight_layout()
    os.chdir(path_save)
    plt.savefig( 'zp_optimum_for_decay_rate_Silver' + labelp + '.png', format='png')  
...
#        fig.tight_layout()
        plt.savefig( 'zp_optimum_for_decay_rate_Silver_sep' + labelp + '.png', format='png')  

    tabla = np.array([list_freq,listy,list_lambda_p])
    tabla = np.transpose(tabla)
    header1 = 'E [meV]     zp [nm]     Re(lambda_p) [nm]' + ', ' + title + ', ' + name_this_py
    np.savetxt('zp_optimum_for_decay_rate_Silver' + labelp + '.txt', tabla, fmt='%1.11e', delimiter='\t', header = header1)
...
if load_data == 1:
    ...
    os.chdir(path_save)
    tabla = np.loadtxt('zp_optimum_for_decay_rate_Silver' + labelp + '.txt' , delimiter='\t', skiprows=1)
    tabla = np.transpose(tabla)
