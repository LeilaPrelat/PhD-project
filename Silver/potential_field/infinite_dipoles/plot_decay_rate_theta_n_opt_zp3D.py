...
tabla = np.loadtxt('zp_optimum_for_decay_rate_Silver_d%inm_v%i_E0%imeV.txt' %(d_nano,int_v,energy0_pol*1e3), delimiter='\t', skiprows=1)
...
if Nmax == 0:
    listx_3 = np.linspace(2,3.5,N)
    list_a_nm = np.array(np.linspace(200,600,N))*1e-3
elif Nmax == 1:
    listx_3 = np.linspace(2,4,N)
    list_a_nm = np.linspace(0.1,1000,len(listx))
    list_a_nm = np.array(np.linspace(50,400,N))*1e-3


elif Nmax == 2:
    listx_3 = np.linspace(1.75,2.7,N)
#    listx_3 = listx_2
    list_a_nm = np.linspace(0.1,1000,len(listx))
    list_a_nm = np.array(np.linspace(50,600,N))*1e-3
...
labelp = r'_zp_opt_Nmax%i_v%i_E0%imeV_theta%i' %(Nmax,int_v,energy0_pol*1e3,theta)
label1 = ''  + labelp
