...
#    
    ...
    kz2 =  lambda u: np.sqrt(epsi_HBN_par - u**2)
    kz3 =  lambda u: np.sqrt(epsi3 - u**2) if (u**2 <= epsi3) else 1j*np.sqrt(u**2 - epsi3)
#
#    kz1 = lambda u : -kz1(u) if np.imag(kz1(u)) < 0 
#    kz2 = lambda u : -kz2(u) if np.imag(kz2(u)) < 0 
#    kz3 = lambda u : -kz3(u) if np.imag(kz3(u)) < 0 


    r12 =  lambda u: (kz1(u)*epsi_HBN_par - kz2(u)*epsi1)/(kz1(u)*epsi_HBN_par + kz2(u)*epsi1)
    r21 = lambda u: (kz2(u)*epsi1 - kz1(u)*epsi_HBN_par)/(kz2(u)*epsi1 + kz1(u)*epsi_HBN_par)
    r23 =  lambda u: (kz2(u)*epsi3 - kz3(u)*epsi_HBN_par)/(kz2(u)*epsi3 + kz3(u)*epsi_HBN_par)
    

    exp_fresnel = lambda u: np.exp(1j*2*kz2(u)*omegac*d_micros)
    
    cte_t = np.sqrt(epsi1*epsi_HBN_par)
    t12 = lambda u: 2*kz1(u)*cte_t/(kz1(u)*epsi_HBN_par + kz2(u)*epsi1)
    t21 = lambda u: 2*kz2(u)*cte_t/(kz2(u)*epsi1 + kz1(u)*epsi_HBN_par)
