...
#    theta = theta_degree*np.pi/180
    ...
    labelp = r'_a%inm_zp_opt_d%inm' %(a*1e3,d_nano)
...
#    
    ...
    ind1,ind2 = 0,-1
    listx_2 = np.linspace(listx[ind1], listx[ind2], 500)
...
def function_real_ana(zp_nano,energy0_eV,Nmax):
...
#        list_y_re = savgol_filter(list_y_re, 101, 3)
        maxi = np.max(list_y_re)
        maxis.append(maxi)
    #    print(n,maxi)
#        list_y_re = np.array(list_y_re)/np.max(maxis)
...
#    plt.yscale('log')
    os.chdir(path_save)
    plt.savefig('decay_rate_' + label1 + '.png', format='png',bbox_inches='tight',pad_inches = 0.01,dpi = dpi)
