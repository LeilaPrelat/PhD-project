...
def green_self_pole_aprox_parallel_num(omegac,epsi1,epsi3,d_nano,zp):
    """
    Parameters
    ----------
    omegac : omega/c = k0 en 1/micrometros    
...
def green_self_ana_parallel_num(omegac,epsi1,epsi3,d_nano,zp_micro):      # mismo que hBN disks pero multiplicado por Rp
    """    
    Parameters
    ----------
    omegac : omega/c = k0 en 1/micrometros    
...
#    alfa_p = np.real(alfa_p)
    ...
    rtaself_x = Rp*0.5*( 2*(2*zp_micro)**(-3) + kp*(2*zp_micro)**(-2) + kp**2*dif_term   )
